...
from .functions.admdash_f.admdash_imp import (
    tk,
    ttk,
    messagebox,
    Path,
...
class AdminDashboard:
    ...
    def show_skeleton_screen(self, rows=8, cols=None):
        ...
        self.scroll_y = ttk.Scrollbar(
            self.table_frame, orient=tk.VERTICAL, style="Custom.Vertical.TScrollbar"
        )
    ...
    def __init__(self, username=None):
        ...
        self.header = tk.Frame(
            self.root, bg="#000000", height=100
        ...
        try:
            logo_path = (
                Path(__file__).resolve().parent.parent / "assets" / "adm_dash.png"
            )
            ...
            self.logo_label = tk.Label(
                header_left, image=self.logo_image, bg="#000000", cursor="hand2"
            )
            self.logo_label.pack(side=tk.LEFT, padx=10, pady=5)
            self.logo_label.bind(
                "<Button-1>", lambda event: self.go_back_to_admin_login()
            )
        except Exception as e:
        ...
        stats = add_stats_panel(
            self.sidebar,
            self.table if hasattr(self, "table") else None,
        ...
        style.map(
            "MacOS.TButton",
            background=[
                ("active", macos_button_active_bg),
                ("pressed", macos_button_active_bg),
            ],
            foreground=[("active", macos_button_fg), ("pressed", macos_button_fg)],
            bordercolor=[
                ("focus", macos_button_border),
                ("!focus", macos_button_border),
            ],
        )
    ...
    def build_sidebar_buttons(self):
        ...
        self.delete_button = tk.Button(
            ...
            command=lambda: delete_item(
                self.table, self.db, self.update_stats, self.username
            ),
            bg="#b71c1c",
            fg="#fffde7",
            activebackground="#0d1331",
        ...
        settings_img_path = os.path.abspath(
        ...
        def settings_with_2fa():
            ...
            if (
                ...
                and self.username
                and self.username.lower() == "bypass"
            ):
            ...
            try:
                ...
                if not secret_enc:
                    if (
                        ...
                        return
                    self.show_toast(
                        "error", "2FA Error", "2FA secret not found for this user."
                    )
                    return
                ...
                if (
                    ...
                    return
                self.show_toast(
                    "error", "2FA Error", f"Failed to decrypt 2FA secret: {e}"
                )
                return
        ...
        add_tooltips(
            {
                ...
                getattr(
                    self, "delete_log_button", None
                ): "Delete the selected log entry"
                if hasattr(self, "delete_log_button")
                else None,
                getattr(self, "settings_btn", None): "Open settings (2FA required)"
    ...
    def create_search_bar_beside_tabs(self):
        ...
        try:
            ...
            if hasattr(self, "tabs_and_search_frame"):
                self.search_container, self.search_entry, self.search_button = (
                    create_rounded_search_bar(
                        self.tabs_and_search_frame,
                        lambda: search_items(self)
                        if self.root and self.root.winfo_exists()
                        else None,
                ...
                self.search_entry.bind(
                    "<KeyRelease>",
                    lambda event: search_items(self, event)
                    if self.root and self.root.winfo_exists()
                    else None,
                )
                ...
                )  # Increased left padding for longer search bar
    ...
    def init_suggestions_popup(self):
        ...
        try:
            ...
            self.suggestions_listbox.bind(
                "<<ListboxSelect>>", lambda event: select_suggestion(self, event)
            )
            self.suggestions_listbox.bind(
                "<Return>", lambda event: select_suggestion(self, event)
            )
            self.suggestions_listbox.bind(
    ...
    def update_datetime(self):
        ...
        try:
            if (
                not hasattr(self, "datetime_label")
                or not self.root
            ...
            if self.root and self.root.winfo_exists():
                self.update_datetime_id = self.root.after(
                    1000, self._delayed_update_datetime
                )
    ...
    def handle_error(self, error_message, retry_callback=None):
        ...
        if retry_callback:
            tk.Button(
                error_window,
                text="Retry",
                command=lambda: [retry_callback(), error_window.destroy()],
                font=("Arial", 12, "bold"),
                bg="#4CAF50",
                fg="white",
            ).pack(pady=10)
    ...
    def load_data(self, preserve_checked_ids=None):
        ...
        def finish_loading():
            ...
            if (
                preserve_checked_ids
                and hasattr(self.table, "toggle_checkbox")
                and hasattr(self.table, "is_checked")
            ):
                for row_id in self.table.get_children(""):
                    values = self.table.item(row_id, "values")
                    if (
                        values
                        and str(values[0]) in preserve_checked_ids
                        and not self.table.is_checked(row_id)
                    ):
    ...
    def view_admin_logs(self):
        ...
        view_admin_logs(self, lambda _: None)
        from gui.functions.admdash_f.ML.search_bar import update_search_bar_visibility
    ...
    def set_current_view(self, view_name):
        ...
        if view_name in ("Admin Logs", "Employee Logs"):
            ...
            if (
                hasattr(self, "add_button")
                and hasattr(self, "update_item_button")
                and hasattr(self, "delete_button")
            ):
                hide_main_buttons(
                    self.add_button, self.update_item_button, self.delete_button
                )
            ...
            elif view_name == "Employee Logs" and hasattr(
                self, "clear_employee_logs_button"
            ):
    ...
    def open_update_window(self):
        ...
        selected_items = [
            item_id for item_id in selected_items if item_id in valid_items
        ]
        if not selected_items:
            self.show_toast(
                "warning", "No Selection", "Please select at least one item to update."
            )
            return
        ...
        for item_id in selected_items:
            try:
                ...
                if len(item_data) > 14 and (
                    item_data[14] is None
                    or str(item_data[14]).strip().lower() == "none"
                    or str(item_data[14]).strip() == ""
                ):
                    from datetime import datetime
        ...
        if missing_ids:
            self.show_toast(
                "warning",
                "Missing Items",
                "Some selected items could not be found and will be skipped.",
            )
        ...
        if items_data:
            ...
            def clear_selection():
                ...
                if not self.root or not self.root.winfo_exists():
                    print(
                        "[DEBUG] Main window no longer exists, skipping post-update cleanup"
                    )
                    return
                ...
                try:
                    if hasattr(self, "table") and self.table:
                        if hasattr(self.table, "clear_checked") and callable(
                            self.table.clear_checked
                        ):
                ...
                try:
                    if not self.root or not self.root.winfo_exists():
                        print(
                            "[DEBUG] Cannot refresh view, root window no longer exists"
                        )
                        return

                    if (
                        hasattr(self, "current_view")
                        and self.current_view == "Restock List"
                    ):
                        print(
                            "[DEBUG] Safely refreshing restock list after closing update window..."
                        )
                        # Use direct call instead of after to avoid callback issues
                        self.view_restock_list()
                    else:
                        print(
                            "[DEBUG] Safely refreshing items list after closing update window..."
                        )
        ...
        def reload_and_clear():
            ...
            try:
                self.root.after(
                    150,
                    lambda: (
                        self.current_view != "Restock List"
                        and self.refresh_items_view()
                    )
                    or None,
                )
            except Exception:
    ...
    def delete_item(self):
        ...
        if hasattr(self.table, "get_checked"):
            ...
            try:
                delete_item(
                    self.table, self.db, update_stats_and_refresh, self.username
                )
            finally:
            ...
            valid_items = set(self.table.get_children())
            selected_items = [
                item_id for item_id in self.table.selection() if item_id in valid_items
            ]
    ...
    def view_total_cost(self):
        ...
        try:
            ...
            self.show_toast(
                "info", "Total Cost", f"The total cost is: ₱{total_cost:,.2f}"
            )
        except Exception as e:
            self.show_toast(
                "error", "Calculation Error", f"Failed to calculate total cost: {e}"
            )

    def go_back_to_admin_login(self):
        """Navigate back to the AdminLogin window."""
        # Sound removed
        from gui.admin_login import (
            AdminLogin,
    ...
    def view_items(self):
        try:
            ...
            from gui.functions.admdash_f.ML.search_bar import (
                update_search_bar_visibility,
            )
    ...
    def _safe_reset_table(self):
        ...
        try:
            ...
            if hasattr(self, "table") and self.table and self.table.winfo_exists():
                self.table.destroy()
            if (
                hasattr(self, "scroll_y")
                and self.scroll_y
                and self.scroll_y.winfo_exists()
            ):
                self.scroll_y.destroy()
            self.table = None
            self.scroll_y = None
    ...
    def show_export_buttons(self):
        show_export_buttons(
            self.export_button, self.export_csv_button, self.current_view
        )
    ...
    def show_feature_message(self, message):
        ...
        if self.root and self.root.winfo_exists():
            self.root.after(
                500, _delayed_bind_sorting
            )  # Delay to ensure table is created

    def delete_log_entry(self):
        """Delete the selected log entry from the database and refresh the Treeview."""
        from backend.database import get_connector, get_db_path
        ...
        if not selected:
            # Sound removed
            self.show_toast(
                "warning", "No Selection", "Please select a log entry to delete."
            )
            return
        ...
        if self.current_view == "Admin Logs":
            ...
            self.show_toast(
                "warning",
                "Invalid View",
                "Log deletion is only available in Admin Logs or Employee Logs view.",
            )
            return

        # Sound removed before confirmation
        if not messagebox.askyesno(
            "Confirm Delete", "Are you sure you want to delete this log entry?"
        ):
            return
        ...
        try:
            ...
            # Assuming row_values contains [DATE, TIME, NAME, DETAILS] or similar
            if len(row_values) >= 4:
                date_val, time_val, name_val, details_val = row_values[:4]
                query = f"DELETE FROM [{table_name}] WHERE [DATE]=? AND [TIME]=? AND [NAME]=? AND [DETAILS]=?"
                params = (date_val, time_val, name_val, details_val)
            elif len(row_values) >= 3:
                time_val, name_val, details_val = row_values[:3]
                query = f"DELETE FROM [{table_name}] WHERE [TIME]=? AND [NAME]=? AND [DETAILS]=?"
                params = (time_val, name_val, details_val)
            else:
                self.show_toast(
                    "error", "Delete Error", "Unable to identify log entry uniquely."
                )
                return
