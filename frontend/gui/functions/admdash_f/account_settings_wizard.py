...
class AccountSettingsWizard:
    ...
    def __init__(self, parent_frame, parent_app, width=1400, height=841):
        ...
        self.key_path = os.path.abspath(
            os.path.join(base, "../../../config/fernet_key.py")
        )

        # Root container
        self.container = tk.Frame(
            parent_frame, bg="#000000", width=self.width, height=self.height
        )
        ...
        self.status_label = tk.Label(
            self.footer, text="", font=("Segoe UI", 10), bg="#000000", fg="#888888"
        )
        self.status_label.pack(side=tk.LEFT)

        # Buttons
        self.edit_start_btn = ttk.Button(
            self.footer,
            text="Edit Account",
    ...
    def _build_overview_step(self):
        ...
        tk.Label(
            center,
            text="Click 'Edit Account' to modify details and access other steps.",
            font=("Segoe UI", 10),
            bg="#101010",
            fg="#999999",
        ).pack(pady=(18, 0))
    ...
    def _build_profile_step(self):
        ...
        tk.Label(
            c,
            text="Update identification details.",
            font=("Segoe UI", 10),
            bg="#101010",
            fg="#999999",
        ).pack(pady=(14, 0))
    ...
    def _build_security_step(self):
        ...
        tk.Label(
            form, text="Password", bg="#101010", fg="#fffde7", font=("Segoe UI", 11)
        ...
        ttk.Button(
            ...
            style="Accent.Rounded.TButton",
        ).grid(row=0, column=3, pady=6, padx=(4, 0))
        self.strength_bar = tk.Canvas(
            form, width=260, height=10, bg="#202020", highlightthickness=0
        )
    ...
    def _add_entry(self, parent, label, var, row):
        tk.Label(
            parent, text=label, bg="#101010", fg="#fffde7", font=("Segoe UI", 11)
    ...
    def _render_progress(self, active=0):
        ...
        for i, _ in enumerate(self.steps):
            ...
            dot.create_text(
                14, 14, text=str(i + 1), fill="#fffde7", font=("Segoe UI", 10, "bold")
            )
            dot.pack()
            lbl = tk.Label(
                wrapper,
                text=step_name,
                bg="#000000",
                fg=("#ffcc80" if i == active else "#bbb"),
                font=("Segoe UI", 8),
            )
            lbl.pack()
    ...
    def update_strength(self):
        ...
        self.strength_bar.create_rectangle(
            0, 0, w, 10, fill=colors[score - 1] if score > 0 else "#444444", outline=""
        )
