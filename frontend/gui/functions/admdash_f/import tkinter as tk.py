...
class UserRolesManager:
    def __init__(self, parent_settings, settings_win):
        ...
        # Wizard (single-frame dialog) infrastructure
        self._wizard_frame = None  # root frame sized 1400x841
        self._wizard_content = None  # current page container
        self._wizard_active = False
        self._wizard_prev_focus = None

    # --------------------------- Wizard Framework ---------------------------
    def _ensure_wizard_frame(self):
        """Create the wizard root frame (overlay) if it doesn't exist."""
        if self._wizard_frame and self._wizard_frame.winfo_exists():
            return
        if not self.settings_win:
            return
        root = self.settings_win
        # Overlay parent frame
        self._wizard_frame = tk.Frame(root, bg="#101010", bd=2, relief="solid")
        ...
        def _place():
            try:
                ...
                if rw <= 1 or rh <= 1:  # fallback using screen if not mapped
                    rw, rh = root.winfo_screenwidth(), root.winfo_screenheight()
                x = (rw - w) // 2
                y = (rh - h) // 2
                self._wizard_frame.place(x=x, y=y, width=w, height=h)
            except Exception:
                pass
    ...
    def _show_wizard_page(self, build_func, title="Wizard", **kwargs):
        """Show a wizard page built by build_func(frame, close_callback, **kwargs)."""
        self._ensure_wizard_frame()
        if not self._wizard_frame:
            return
        # Clear old content
        for child in self._wizard_frame.winfo_children():
            child.destroy()
        self._wizard_active = True
        self._wizard_prev_focus = (
            self.settings_win.focus_get() if self.settings_win else None
        )

        # Header bar
        header = tk.Frame(self._wizard_frame, bg="#181818")
        header.pack(fill="x")
        tk.Label(
            header,
            text=title,
        ...
        content = tk.Frame(self._wizard_frame, bg="#101010")
        content.pack(fill="both", expand=True, padx=10, pady=10)
        self._wizard_content = content

        build_func(content, close_wizard, **kwargs)

    def _hide_wizard(self):
        if self._wizard_frame and self._wizard_frame.winfo_exists():
            self._wizard_frame.place_forget()
        self._wizard_active = False
        if self._wizard_prev_focus:
            try:
                self._wizard_prev_focus.focus_set()
            except Exception:
                pass

    # ---------------------- Refactored Dialog Pages ------------------------
    def _page_authenticate_demotion(
        self,
        parent,
        close_cb,
        target_username,
        ...
        tk.Entry(
            ...
            fg="#ffffff",
            insertbackground="#ffffff",
        ).grid(row=3, column=0, pady=(2, 20), sticky="w")
        ...
        def do_auth():
            ...
            if not self.verify_target_user_credentials(target_username, password):
                ...
                return
            if otp_code:
                if not self.verify_target_user_credentials_flexible(
                    target_username, password, otp_code
                ):
    ...
    def _page_2fa_info(self, parent, close_cb, username, otp_secret):
        ...
        tk.Label(
            wrapper,
            text="Secret (store securely):",
            font=("Segoe UI", 11, "bold"),
            bg="#101010",
            fg="#ffcc00",
        ).pack(pady=(20, 4))
        secret_box = tk.Text(
            wrapper, height=2, width=50, bg="#1e1e1e", fg="#00ff88", relief="flat"
        )
        ...
        tk.Label(
            wrapper,
            text="Add this secret to an Authenticator app (TOTP).",
            font=("Segoe UI", 10),
            bg="#101010",
            fg="#cccccc",
        ).pack(pady=10)
    ...
    def create_full_interface(self, parent):
        """Create full interface for Level 3 users"""
        # Main container
        container = tk.Frame(parent, bg="#000000")
        container.pack(fill="both", expand=True, padx=18, pady=8)

        # Instructions
        instructions_frame = tk.Frame(container, bg="#000000")
        instructions_frame.pack(fill="x", pady=(0, 10))

        instructions_text = (
            "Instructions: Right-click on any user to change their access level."
        )
        tk.Label(
            instructions_frame,
        ...
        # Search frame
        search_frame = tk.Frame(container, bg="#000000")
        ...
        search_entry.pack(side="left", padx=(10, 0), ipady=3)
        ...
        self.stats_label = tk.Label(
            stats_frame, text="", font=("Segoe UI", 9), bg="#000000", fg="#999"
        )
        self.stats_label.pack(anchor="w")

        # Table frame with scrollbar
        table_frame = tk.Frame(container, bg="#000000")
        table_frame.pack(fill="both", expand=True, pady=(0, 10))

        # Create Treeview for the table
        columns = ("Last Name", "First Name", "Access Level")
        ...
        # Configure style for modified rows
        style.configure(
            "Modified.Treeview",
            background="#2d4a3e",  # Darker green background
            foreground="#a8d8a8",  # Light green text
            fieldbackground="#2d4a3e",
        )

        # Map the modified style
        style.map(
            "Modified.Treeview", background=[("selected", "#4a6741")]
        )  # Selected modified row color

        # Configure column headings with sorting
        self.user_tree.heading(
            "Last Name",
            text="Last Name ↕",
            command=lambda: self.sort_treeview("Last Name"),
        )
        self.user_tree.heading(
            "First Name",
            text="First Name ↕",
            command=lambda: self.sort_treeview("First Name"),
        )
        self.user_tree.heading(
            "Access Level",
            text="Access Level ↕",
            command=lambda: self.sort_treeview("Access Level"),
        )
        ...
        scrollbar = ttk.Scrollbar(
            table_frame, orient="vertical", command=self.user_tree.yview
        )
        ...
        # Buttons frame (macOS style)
        btn_frame = tk.Frame(container, bg="#000000")
        btn_frame.pack(anchor="e", pady=(10, 0))

        # Employee management buttons frame (left side)
        emp_mgmt_frame = tk.Frame(btn_frame, bg="#000000")
        emp_mgmt_frame.pack(side=tk.LEFT, padx=(0, 20))

        # Add Employee button
        self.add_emp_btn = tk.Button(
            emp_mgmt_frame,
            text="Add Employee",
            font=("Segoe UI", 10),
            bg="#28a745",
        ...
        self.remove_emp_btn = tk.Button(
            ...
            activeforeground="#fff",
            width=12,
            command=self.remove_employee_dialog,
        )
        self.remove_emp_btn.pack(side=tk.LEFT, padx=(0, 5))

        # Edit Employee button
        self.edit_emp_btn = tk.Button(
            emp_mgmt_frame,
            text="Edit Employee",
            font=("Segoe UI", 10),
            bg="#ffc107",
        ...
        self.bulk_ops_btn = tk.Button(
            ...
            fg="#fff",
            relief="flat",
            activebackground="#5a36a1",
            activeforeground="#fff",
            width=12,
            command=self.bulk_operations_dialog,
        )
        self.bulk_ops_btn.pack(side=tk.LEFT)

        # Cancel button (right side)
        self.cancel_roles_btn = tk.Button(
            btn_frame,
            text="Cancel",
            font=("Segoe UI", 11),
            bg="#666",
        ...
        # Save button (right side) - starts in disabled state
        self.save_roles_btn = tk.Button(
            btn_frame,
            text="💾 Save Changes",
            font=("Segoe UI", 10, "normal"),
            bg="#6c757d",  # Gray background (disabled state)
            fg="white",
            relief="flat",
            activebackground="#5a6268",
            activeforeground="#fff",
            width=15,
            command=self.save_changes_simplified,
        )
        self.save_roles_btn.pack(side=tk.RIGHT)

        # Bind events for right-click context menu
        self.user_tree.bind(
            "<Button-3>", self.on_right_click
        )  # Right-click context menu

        # Keyboard shortcuts
        self.user_tree.bind("<F5>", lambda e: self.refresh_data())  # F5 to refresh
        self.user_tree.bind("<Delete>", self.delete_key_pressed)  # Delete key to remove
        self.user_tree.bind("<Return>", self.enter_key_pressed)  # Enter key to edit
        self.user_tree.bind(
            "<Control-p>", lambda e: self.show_pending_changes_summary()
        )  # Ctrl+P to show pending changes

        # Double-click to edit
        self.user_tree.bind(
            "<Double-1>",
            lambda e: self.edit_employee_dialog()
            if self.user_tree.selection()
            else None,
        )

        # Add keyboard shortcuts info
        shortcuts_frame = tk.Frame(container, bg="#000000")
        shortcuts_frame.pack(fill="x", pady=(5, 0))

        shortcuts_text = "💡 Shortcuts: F5=Refresh | Delete=Remove | Enter=Edit | Double-click=Edit | Right-click=Context Menu | Ctrl+P=Show Pending Changes"
        ...
        # Add pending changes info
        changes_info_frame = tk.Frame(container, bg="#000000")
        changes_info_frame.pack(fill="x", pady=(2, 0))

        changes_text = "📝 Changes are temporary until saved. Modified rows show ● symbol. Changes tracked: Window title shows * when unsaved."
        ...
        # Schedule initial data load once UI is ready
        try:
            target = (
                self.settings_win
            ...
            target.after(0, self.load_user_data)
        except Exception:
            pass
    ...
    def sort_treeview(self, column):
        ...
        try:
            ...
            column_index = {"Last Name": 0, "First Name": 1, "Access Level": 2}

            # Sort the data
            if column in column_index:
                idx = column_index[column]
                items_data.sort(
                    key=lambda x: str(x[1][idx]).lower(), reverse=self.sort_reverse
                )
    ...
    def load_user_data(self):
        ...
        if not self.user_tree:
            ...
            # Schedule a retry after a short delay if the tree isn't ready yet
            if hasattr(self, "settings_win") and self.settings_win:
                self.settings_win.after(50, self.load_user_data)
            return

        print("[DEBUG] user_tree is available, proceeding with data load")
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                item_id = self.user_tree.insert(
                    "", "end", values=(last_name, first_name, access_level)
                )
            ...
            try:
                messagebox.showerror("Load Error", f"Failed to load user data: {e}")
            except Exception:
                pass
        finally:
            try:
    ...
    def save_changes(self):
        ...
        for item_id in self.user_tree.get_children():
            ...
            if item_id in self.original_user_data:
                original_data = self.original_user_data[item_id]
                current_level = values[2]  # Access Level column
        ...
        print(f"[DEBUG] Found {len(changes)} changes to save")
        for change in changes:
            print(
                f"[DEBUG] Change: {change['first_name']} {change['last_name']} ({change['old_level']} -> {change['new_level']})"
            )

        # Check for Level 3 protection - prevent last Level 3 user from demoting themselves
        current_username = getattr(self.parent_settings.parent, "username", None)
        level_3_demotions = [
            c
            for c in changes
            if c["old_level"] == "Level 3" and c["new_level"] != "Level 3"
        ]
        self_demotion = [
            c for c in level_3_demotions if c["username"] == current_username
        ]

        if self_demotion:
            # Count Level 3 users after all changes
        ...
        demotion_changes = [
            c
            for c in changes
            if c["old_level"] in ["Level 2", "Level 3"] and c["new_level"] == "Level 1"
        ]
        promotion_changes = [
            c
            for c in changes
            if c["old_level"] == "Level 1" and c["new_level"] in ["Level 2", "Level 3"]
        ]
        ...
        try:
            ...
            for i, change in enumerate(changes):
                ...
                if (
                    change["old_level"] in ["Level 2", "Level 3"]
                    ...
                    print(
                        f"[DEBUG] Cleared credentials for demoted user: {change['username']}"
                    )
                else:
            ...
            demoted_users = [
                c
                for c in changes
                if c["old_level"] in ["Level 2", "Level 3"]
                and c["new_level"] == "Level 1"
            ]
            ...
            if demoted_users:
                ...
                messagebox.showinfo(
                    "Success", f"Successfully updated {len(changes)} user(s)."
                )
    ...
    def authenticate_sensitive_changes(self, sensitive_changes):
        ...
        for change in sensitive_changes:
            ...
            if not self.authenticate_user_demotion(
                change["username"], user_name, change["old_level"], change["new_level"]
            ):
                return False
        return True

    def authenticate_user_demotion(
        self, target_username, user_display_name, old_level, new_level
    ):
        ...
        tk.Label(
            title_frame, text="⚠️", font=("Segoe UI", 24), bg="#000000", fg="#ff6f00"
        ).pack()
        tk.Label(
            title_frame,
            text="Credential Surrender Required",
...
The user being demoted must provide their credentials to surrender their current access level."""
        ...
        tk.Label(
            info_frame,
            text=f"Access Level Change: {old_level} → {new_level}",
            font=("Segoe UI", 11),
            bg="#000000",
            fg="#ffcc00",
        ).pack(pady=(0, 10))
        ...
        tfa_radio = tk.Radiobutton(
            radio_frame,
            text="📱 2FA Code Authentication",
            variable=auth_method,
            value="2fa",
            bg="#000000",
            fg="#fffde7",
        ...
        password_label = tk.Label(
            cred_frame,
            text=f"🔑 {user_display_name}'s Password:",
            font=("Segoe UI", 11, "bold"),
            bg="#000000",
            fg="#fffde7",
        )
        ...
        tfa_label = tk.Label(
            cred_frame,
            text=f"📱 {user_display_name}'s 2FA Code:",
            font=("Segoe UI", 11, "bold"),
            bg="#000000",
            fg="#fffde7",
        )
...
• The user must be present to provide their credentials"""
        ...
        def verify_auth():
            ...
            if selected_method == "password":
                if not password:
                    messagebox.showerror(
                        "Input Required", "Please enter the user's password."
                    )
                    return
                ...
                if not tfa_code:
                    messagebox.showerror(
                        "Input Required", "Please enter the user's 2FA code."
                    )
                    return
                # Clear password field to ensure only 2FA is used
                password = ""

            if self.verify_target_user_credentials_flexible(
                target_username, password, tfa_code, selected_method
            ):
                auth_result["success"] = True
                auth_win.destroy()
            else:
                method_name = (
                    "password" if selected_method == "password" else "2FA code"
                )
                messagebox.showerror(
    ...
    def verify_target_user_credentials_flexible(
        ...
        try:
            ...
            if auth_method == "password":
                ...
                if not password:
                    return False
                if not row[0]:  # No password stored
                    return False
                try:
                ...
                if not tfa_code:
                    return False
                if not row[1]:  # No 2FA secret stored
                    return False
                try:
    ...
    def cancel_changes(self):
        ...
        if self.has_pending_changes:
            ...
            if result:
                print(
                    f"[DEBUG] User confirmed discarding {change_count} pending changes"
                )
                self.load_user_data()
            else:
    ...
    def authenticate_admin_promotions(self, promotion_changes):
        ...
        if not current_username:
            messagebox.showerror(
                "Error", "Unable to identify current user for authentication."
            )
            return False
        ...
        for change in promotion_changes:
            ...
            promotion_summary.append(
                f"• {user_name}: {change['old_level']} → {change['new_level']}"
            )

        return self.authenticate_admin_promotion(
            current_username, admin_display_name, promotion_summary
        )

    def authenticate_admin_promotion(
        self, admin_username, admin_display_name, promotion_summary
    ):
        ...
        auth_win.update_idletasks()
        x = (
            self.settings_win.winfo_rootx()
            + (self.settings_win.winfo_width() // 2)
            - 325
        )
        y = (
            self.settings_win.winfo_rooty()
            + (self.settings_win.winfo_height() // 2)
            - 275
        )
        auth_win.geometry(f"650x550+{x}+{y}")
        ...
        tk.Label(
            title_frame, text="🔐", font=("Segoe UI", 24), bg="#000000", fg="#00a8ff"
        ).pack()
        tk.Label(
            title_frame,
        ...
        tk.Label(
            admin_section,
            text=f"Administrator: {admin_display_name}",
            font=("Segoe UI", 12, "bold"),
            bg="#1c1c1c",
            fg="#fffde7",
        ).pack()
        ...
        tk.Label(
            promo_section,
            text="User Promotions to Authorize:",
            font=("Segoe UI", 11, "bold"),
            bg="#1c1c1c",
            fg="#00d9ff",
        ).pack(pady=(0, 8))
        ...
        tfa_var = tk.StringVar()
        tfa_entry = tk.Entry(
            cred_frame, textvariable=tfa_var, font=("Segoe UI", 11), width=40
        )
        ...
        def verify_admin_auth():
            ...
            if selected_method == "password":
                ...
                if not tfa_code:
                    messagebox.showerror(
                        "Input Required", "Please enter your 2FA code."
                    )
                    return
                # Clear password field to ensure only 2FA is used
                password = ""
            ...
            if self.verify_target_user_credentials_flexible(
                admin_username, password, tfa_code, selected_method
            ):
                auth_result["success"] = True
                auth_win.destroy()
            else:
    ...
    def save_changes_simplified(self):
        ...
        for change in promotion_changes:
            print(
                f"[DEBUG] Setting up promoted user: {change['first_name']} {change['last_name']}"
            )
            ...
            if not self.test_database_connection():
                messagebox.showerror(
                    "Database Error",
                    "Cannot connect to database. Please check the connection and try again.",
                )
                return
            ...
            print(
                f"[DEBUG] Setup completed for {change['username']}: new username = {setup_result['username']}"
            )
        ...
        sensitive_changes = [
            ...
            for c in changes
            if c["old_level"] in ["Level 2", "Level 3"]
            or c["new_level"] in ["Level 2", "Level 3"]
        ]
        ...
        try:
            ...
            for i, change in enumerate(changes):
                ...
                # Check if this is a promotion with credential setup
                    ...
                    print(
                        f"[DEBUG] Processing promotion setup for: {setup_info['username']}"
                    )
                    ...
                    try:
                        ...
                        if not cred_result.get("success", False):
                            error_msg = cred_result.get(
                                "error", "Unknown error during credential update"
                            )
                            raise Exception(
                                f"Failed to update credentials for {change['username']}: {error_msg}"
                            )
                        ...
                        print(
                            f"[DEBUG] Updated promoted user: {setup_info['username']} -> {change['new_level']}"
                        )
                        ...
                        print(
                            f"[DEBUG] Error in credential update process: {cred_error}"
                        )
                        raise cred_error
            ...
            for change in changes:
                if "show_2fa_info" in change:
                    try:
                        print(
                            f"[DEBUG] Showing 2FA setup info for: {change['show_2fa_info']['username']}"
                        )
            ...
            promoted_users = [
                c
                for c in changes
                if c["old_level"] == "Level 1"
                and c["new_level"] in ["Level 2", "Level 3"]
            ]
    ...
    def on_right_click(self, event):
        ...
        try:
            ...
            menu.add_command(
                label="Remove", command=lambda: self.remove_employee_dialog()
            )
            ...
            def change_level(new_level):
                ...
                self.pending_changes[item_id] = {
                    ...
                    "username": self.original_user_data.get(item_id, {}).get(
                        "username", ""
                    ),
            ...
            if current_level == "Level 1":
                menu.add_command(
                    label="Promote to Level 2", command=lambda: change_level("Level 2")
                )
                menu.add_command(
                    label="Promote to Level 3", command=lambda: change_level("Level 3")
                )
                ...
                menu.add_command(
                    label="Demote to Level 1", command=lambda: change_level("Level 1")
                )
            elif current_level == "Level 3":
                menu.add_command(
                    label="Demote to Level 2", command=lambda: change_level("Level 2")
                )
                menu.add_command(
                    label="Demote to Level 1", command=lambda: change_level("Level 1")
                )
            ...
            menu.add_separator()
            menu.add_command(
                label="Show Pending Changes", command=self.show_pending_changes_summary
            )
            ...
            print(f"[DEBUG] Error in on_right_click: {e}")
    ...
    def filter_users(self, *args):
        ...
        try:
            ...
            for item_id, user_data in self.all_user_data.items():
                ...
                if (
                    not search_query
                    or search_query in first_name
                    or search_query in last_name
                    or search_query in username
                    or search_query in access_level
                ):
            ...
            for user_data in matching_items:
                new_item_id = self.user_tree.insert(
                    "",
                    "end",
                    values=(
                        user_data["last_name"],
    ...
    def bulk_operations_dialog(self):
        ...
        try:
            ...
            employee_tree = ttk.Treeview(
                selection_frame, columns=columns, show="headings", height=10
            )
            ...
            # Scrollbar
            scrollbar_bulk = ttk.Scrollbar(
                selection_frame, orient="vertical", command=employee_tree.yview
            )
            ...
            def load_employee_data():
                ...
                current_username = getattr(
                    self.parent_settings.parent, "username", None
                )
                ...
                for item_id, user_data in self.original_user_data.items():
                    ...
                    if (
                        current_username
                        and username.lower() == current_username.lower()
                    ):
                        continue

                    employee_tree.insert(
                        "", "end", values=("☐", name, username, access_level)
                    )

            # Toggle selection
            def toggle_selection(event):
                item = (
                    employee_tree.selection()[0] if employee_tree.selection() else None
                )
                if item:
            ...
            def execute_bulk_operation():
                if not selected_items:
                    messagebox.showwarning(
                        "No Selection", "Please select at least one employee."
                    )
                    return
                    ...
                    if messagebox.askyesno("Confirm Bulk Operation", confirm_msg):
                        ...
                        for item in selected_items:
                            ...
                            for (
                                orig_item_id,
                                orig_user_data,
                            ) in self.original_user_data.items():
                        ...
                        if success_count > 0:
                            result_msg = (
                                f"Successfully updated {success_count} employee(s)."
                            )
                            if failed_users:
                                result_msg += (
            ...
            execute_btn = tk.Button(
                ...
                activeforeground="#fff",
                width=15,
                command=execute_bulk_operation,
            )
...
• Username must be unique and 2+ characters"""
            ...
            def validate_and_add():
                ...
                validation_errors = self.validate_user_input(
                ...
                # Add the employee
                success, message = self.add_employee_to_database(
            ...
            entries["First Name:"].bind(
                "<Return>", lambda e: entries["Last Name:"].focus()
            )
            entries["Last Name:"].bind(
                "<Return>", lambda e: entries["Username:"].focus()
            )
            entries["Username:"].bind(
                "<Return>", lambda e: entries["Password:"].focus()
            )
    ...
    def remove_employee_dialog(self):
        ...
        try:
            ...
            if current_username and username.lower() == current_username.lower():
                messagebox.showwarning(
                    "Cannot Remove", "You cannot remove your own account."
                )
                return
            ...
            if success:
                messagebox.showinfo(
                    "Success", f"Employee {display_name} has been removed successfully!"
                )
                self.load_user_data()  # Refresh the user list
            else:
    ...
    def edit_employee_dialog(self):
        ...
        try:
            ...
            if not selected:
                messagebox.showwarning(
                    "No Selection", "Please select an employee to edit."
                )
                return
...
• Changes are applied immediately after clicking Update"""
            ...
            for field_name, var, readonly in fields:
                ...
                username_bg = (
                    "#2a2a3e"
                    if field_name == "Username:" and is_current_user
                    else "#23272b"
                )
                ...
                if field_name == "Username:":
                    username_status_label = tk.Label(
                        field_frame,
                        text="",
                        font=("Segoe UI", 9),
                        bg="#000000",
                    ...
                    if is_current_user:
                        current_user_label = tk.Label(
                            field_frame,
                            text="👤 This is your own username",
                    ...
                    def check_username_availability(*args):
                        ...
                        if not new_username:
                            ...
                            username_status_label.config(
                                text="✗ Username already exists", fg="#f44336"
                            )
                        else:
                            username_status_label.config(
                                text="✓ Username available", fg="#4CAF50"
                            )
                ...
                if username_changed:
                    ...
                    if self.username_exists(new_username, original_username):
                        validation_label.config(
                            text="Error: Username already exists. Please choose a different username."
                        )
                        return
                    ...
                    if current_user and current_user == original_username:
                        ...
                        tk.Label(
                            warning_frame,
                            text="⚠️ Username Change Confirmation",
                            font=("Segoe UI", 14, "bold"),
                            bg="#000000",
                            fg="#ff9800",
                        ).pack(pady=(0, 15))
...
Are you absolutely sure you want to proceed?"""
                        ...
                        proceed_with_change = [
                            False
                        ]  # Use list to modify from inner function
                        ...
                        tk.Button(
                            ...
                            activeforeground="#fff",
                            width=12,
                            command=cancel_username_change,
                        ...
                        tk.Button(
                            ...
                            activeforeground="#fff",
                            width=18,
                            command=confirm_username_change,
                        ).pack(side=tk.RIGHT)
                ...
                if success:
                    ...
                    if (
                        username_changed
                        and current_user
                        and current_user == original_username
                    ):
                        messagebox.showinfo(
                            "Success",
    ...
    # NOTE: Removed duplicate helper method definitions (username_exists, add/remove/update employee,
    # handle_current_user_logout, cancel_changes, save_changes_simplified) that appeared a second time at
    # end of file and overrode earlier, more feature-complete implementations. This prevents unintended
    # behavior like losing confirmation prompts and advanced save logic.
