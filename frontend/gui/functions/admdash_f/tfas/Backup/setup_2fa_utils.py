...
def setup_2fa(
    ...
    def do_generate_and_show(content, entered_username):
        ...
        uri = pyotp.totp.TOTP(new_secret).provisioning_uri(
            name=entered_username, issuer_name="JJCFPIS"
        )
        ...
        secret_label = tk.Label(
            content, text=f"{new_secret}", font=("Segoe UI", 10), bg=C["bg"], fg=C["fg"]
        )
        secret_label.is_dynamic = True
        secret_label.pack(pady=(2, 2))
        instr_label = tk.Label(
            content,
            text="Scan QR & enter 6-digit code:",
            font=("Segoe UI", 10),
            bg=C["bg"],
            fg=C["fg"],
        )
    ...
    if popup:
        ...
        for color in (C["mac_btn_red"], C["mac_btn_yellow"], C["mac_btn_green"]):
            btn = tk.Canvas(
                btn_frame, width=14, height=14, bg=C["bg"], highlightthickness=0, bd=0
            )
        ...
        close_btn.create_oval(
            2, 2, 12, 12, fill=C["mac_btn_red"], outline=C["mac_btn_red"]
        )
    ...
    tk.Label(
        content, text="Username", font=("Segoe UI", 12), bg=C["bg"], fg=C["fg"]
    ).pack(pady=(8, 2))
    ...
    def on_generate():
        ...
        try:
            ...
            if row_secret:
                messagebox.showinfo(
                    "2FA Exists", "2FA is already set up for this user."
                )
                return
        except Exception as e:
    ...
    close_btn2 = tk.Button(
        content,
        text="Close",
        command=(win.destroy if popup else content.destroy),
