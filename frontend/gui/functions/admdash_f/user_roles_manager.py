...
class UserRolesManager:
    ...
    def _wizard_add_emp_step_details(self, parent, close_cb):
        ...
        for label, key in [
            ...
            tk.Label(
                row, text=label, font=("Segoe UI", 11), bg="#101010", fg="#fffde7"
            ).pack(anchor="w")
        ...
        ttk.Combobox(
            lvl_row,
            textvariable=d["access_level_var"],
        ...
        tk.Button(
            nav,
            text="Next ▶",
            command=lambda: self._add_emp_details_next(),
            bg="#ff6f00",
            fg="#fff",
            relief="flat",
            width=14,
    ...
    def _wizard_add_emp_step_credentials(self, parent, close_cb):
        ...
        tk.Label(
            frm,
            text="Enter a temporary password (user can change later).",
            font=("Segoe UI", 10),
            bg="#101010",
            fg="#bbb",
        ).pack(anchor="w")
        ...
        tk.Entry(
            ...
            relief="flat",
            insertbackground="#fffde7",
        ).pack(fill="x", ipady=6, pady=(12, 0))
        nav = tk.Frame(frm, bg="#101010")
        nav.pack(fill="x", pady=20)
        tk.Button(
            nav,
            text="◀ Back",
            command=self._wizard_go_back,
            bg="#444",
            fg="#fff",
            relief="flat",
            width=12,
        ...
        tk.Button(
            nav,
            text="Next ▶",
            command=lambda: self._wizard_go_next(),
            bg="#ff6f00",
            fg="#fff",
            relief="flat",
            width=14,
    ...
    def _wizard_add_emp_step_confirm(self, parent, close_cb):
        ...
        def finalize():
            ...
            if hasattr(self, "add_employee_to_database"):
                res = self.add_employee_to_database(
                    d["first_name"],
                    d["last_name"],
                    d["username"],
                    pwd,
    ...
    def _wizard_edit_emp_step_details(self, parent, close_cb):
        ...
        tk.Button(
            nav,
            text="Next ▶",
            command=lambda: self._edit_emp_step_details_next(),
            bg="#ff6f00",
            fg="#fff",
            relief="flat",
            width=14,
    ...
    def _wizard_edit_emp_step_confirm(self, parent, close_cb):
        ...
        def apply_changes():
            ...
            res = (
                self.update_employee_in_database(
                    o["username"],
                    d["first_name"],
                    d["last_name"],
                    d["username"] if d["username"] != o["username"] else None,
                    new_pw,
                    d["access_level"],
                )
                if hasattr(self, "update_employee_in_database")
                else (False, "Missing update method")
            )
    ...
    def _wizard_bulk_step_select(self, parent, close_cb):
        ...
        tk.Label(
            frm,
            text="Double-click to toggle selection.",
            font=("Segoe UI", 10),
            bg="#101010",
            fg="#bbb",
        ).pack(anchor="w", pady=(0, 10))
        ...
        tk.Button(
            nav,
            text="Next ▶",
            command=lambda: self._bulk_select_next(),
            bg="#ff6f00",
            fg="#fff",
            relief="flat",
            width=14,
    ...
    def _wizard_bulk_step_configure(self, parent, close_cb):
        ...
        tk.Button(
            nav,
            text="Next ▶",
            command=lambda: self._bulk_config_next(),
            bg="#ff6f00",
            fg="#fff",
            relief="flat",
            width=14,
    ...
    def _bulk_config_next(self):
        self._wizard_flow_data["new_level"] = self._wizard_flow_data[
            "new_level_var"
        ].get()
        self._wizard_go_next()
    ...
    def _wizard_bulk_step_confirm(self, parent, close_cb):
        ...
        def apply_bulk():
            ...
            if successes:
                ...
                messagebox.showinfo(
                    "Bulk Operation",
                    f"Successfully updated {successes} employee(s).{extra}",
                )
                self._hide_wizard()
                self.load_user_data()
            else:
                ...
                messagebox.showerror(
                    "Bulk Operation", f"Failed to update employees: {failure_list}"
                )
    ...
    def create_full_interface(self, parent):
        ...
        tk.Label(
            inst,
            text="Instructions: Right-click on any user to change their access level.",
            font=("Segoe UI", 10),
            bg="#000000",
            fg="#999",
            wraplength=600,
        ...
        self.stats_label = tk.Label(
            stats, text="", font=("Segoe UI", 9), bg="#000000", fg="#999"
        )
        ...
        for c in cols:
            self.user_tree.heading(
                c, text=f"{c} ↕", command=lambda col=c: self.sort_treeview(col)
            )
        ...
        self.add_emp_btn = tk.Button(
            ...
            activeforeground="#fff",
            width=12,
            command=self.launch_add_employee_wizard,
        )
        ...
        self.edit_emp_btn = tk.Button(
            ...
            width=12,
            command=self.launch_edit_employee_wizard,
        )
        ...
        self.bulk_ops_btn = tk.Button(
            ...
            activeforeground="#fff",
            width=12,
            command=self.launch_bulk_operations_wizard,
        )
        ...
        self.user_tree.bind(
            "<Double-1>",
            lambda e: self.launch_edit_employee_wizard()
        ...
        tk.Label(
            sc,
            text="💡 Shortcuts: F5=Refresh | Delete=Remove | Enter=Edit | Double-click=Edit | Right-click=Context Menu | Ctrl+P=Show Pending Changes",
            font=("Segoe UI", 8),
            bg="#000000",
            fg="#666",
        ).pack(anchor="w")
        ...
        tk.Label(
            ci,
            text="📝 Changes are temporary until saved. Modified rows show ● symbol. Window title shows * when unsaved.",
            font=("Segoe UI", 8),
            bg="#000000",
            fg="#888",
        ).pack(anchor="w")
...
Are you absolutely sure you want to proceed?"""
            ...
            try:
                messagebox.showerror(
                    "Error", f"Failed to open edit employee dialog: {e}"
                )
            except Exception:
