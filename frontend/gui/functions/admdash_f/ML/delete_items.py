...
def delete_item(table, db, update_stats, username="Admin"):
    ...
    confirm = messagebox.askyesno(
        "Confirm Delete", "Are you sure you want to delete the selected material(s)?"
    )
    if not confirm:
        return
        ...
        for item_id in selected_items:
            ...
            try:
                ...
                messagebox.showerror(
                    "Error", f"Failed to delete {material_to_delete}: {e}"
                )
                continue
            table.delete(item_id)
            queries.insert_admin_log(
                db,
                username,
                f"Deleted material: {material_to_delete} from supplier: {supplier}",
            )
