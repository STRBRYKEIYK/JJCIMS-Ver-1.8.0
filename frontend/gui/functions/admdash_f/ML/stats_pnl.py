...
class StatsPanel:
    def __init__(self, parent, table, username=None):
        ...
        tk.Label(
            user_frame, text="👤 ", font=("Arial", 10), bg="#181c1f", fg="#4CAF50"
        ).pack(side=tk.LEFT)
        tk.Label(
            user_frame,
            text=self.username,
        ...
        self.datetime_label = tk.Label(
            header_frame, text="", font=("Arial", 10), bg="#181c1f", fg="#666666"
        )
    ...
    def _update_datetime(self):
        ...
        try:
            ...
            if (
                ...
                or not self.parent.winfo_exists()
                or not hasattr(self, "datetime_label")
                or not self.datetime_label.winfo_exists()
            ):
                return
    ...
    def _create_stat_label(self, text):
        ...
        try:
            if (
                self._is_destroyed
                or not hasattr(self, "stats_frame")
                or not self.stats_frame.winfo_exists()
            ):
                return None
    ...
    def _slide_tip(self):
        ...
        try:
            ...
            self.current_tip_index = (self.current_tip_index + 1) % len(
                self.system_tips
            )
            ...
            def slide_out(pos=100):
                try:
                    ...
                    if (
                        not self.parent.winfo_exists()
                        or not self.tip_label.winfo_exists()
                    ):
                        return
    ...
    def update_stats(self, stats):
        ...
        try:
            ...
            self._animate_update(
                self.total_items_label, f"Total Items: {stats['total_items']}"
            )
            self._animate_update(
                self.out_of_stock_label, f"Out of Stock: {stats['out_of_stock']}"
            )
            self._animate_update(
                self.low_stock_label, f"Low in Stock: {stats['low_stock']}"
            )

            if "total_cost" in stats:
                self._animate_update(
                    self.total_balance_label, f"Total Cost: ₱{stats['total_cost']:,.2f}"
                )

            # Update refresh time
            if hasattr(self, "refresh_label") and self.refresh_label.winfo_exists():
                self.refresh_label.config(
                    text=f"Last Refresh: {time.strftime('%I:%M:%S %p')}"
                )
        except Exception as e:
...
def add_stats_panel(parent, table, username=None):
    ...
    return (
        panel.total_items_label,
        panel.out_of_stock_label,
        panel.low_stock_label,
        panel.total_balance_label,
    )
...
def get_db_stats(db_path=None, mode="ITEMS_LIST"):
    ...
    try:
        ...
        if normalized_mode == "ITEMS_LIST":
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM ITEMSDB WHERE Status IN ('Low in Stock', 'Out of Stock')"
            )
    ...
    try:
        ...
        if isinstance(total_items_label, tk.Label):  # Old style update
            if stats:
                ...
                total_balance_label.config(
                    text=f"Total Cost: ₱{stats['total_cost']:,.2f}"
                )
        ...
        if mode in ("ITEMS_LIST", "default") and db_path:
            ...
            if stats:
                if isinstance(total_items_label, tk.Label):  # Old style update
                    ...
                    out_of_stock_label.config(
                        text=f"Out of Stock: {stats['out_of_stock']}"
                    )
            ...
            if stats:
                if isinstance(total_items_label, tk.Label):  # Old style update
                    ...
                    out_of_stock_label.config(
                        text=f"To Restock: {stats['total_restock']}"
                    )
            ...
            if isinstance(total_items_label, tk.Label):  # Old style update
                ...
                empty_stats = {"total_items": 0, "out_of_stock": 0, "low_stock": 0}
