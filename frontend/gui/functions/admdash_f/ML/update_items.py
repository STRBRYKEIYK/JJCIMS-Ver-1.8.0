...
class ToastNotification:
    ...
    def position_toast(self):
        ...
        x = (
            ...
            - self.toast.winfo_reqwidth()
            - 20
        )
        y = self.parent.winfo_y() + 80
        self.toast.geometry(
            f"{self.toast.winfo_reqwidth()}x{self.toast.winfo_reqheight()}+{x}+{y}"
        )
...
def show_custom_dialog(
    parent, title, message, type="info", buttons=None
):  # Deprecated duplicate kept for backward compat
...
class CustomDialog:
    def __init__(self, parent, title, message, type="info", buttons=["OK"]):
        ...
        self.dialog.bind(
            "<Escape>",
            lambda e: self.on_button("Cancel" if "Cancel" in buttons else buttons[-1]),
        )

        # Bind enter key to OK
        self.dialog.bind(
            "<Return>",
            lambda e: self.on_button("OK" if "OK" in buttons else buttons[0]),
        )
...
ASSETS_PATH = (
    Path(__file__).parent.parent.parent.parent.parent / "assets" / "upi_assets"
)
...
class UpdateItemsWindow:
    def __init__(
        self, root, items_data, db_connection, refresh_callback=None, on_close=None
    ):
        ...
        for field_key, db_column in combobox_fields.items():
            try:
                ...
                cursor.execute(
                    f"SELECT DISTINCT {db_column} FROM ITEMSDB WHERE {db_column} IS NOT NULL AND {db_column} <> '' ORDER BY {db_column}"
                )
                values = [row[0] for row in cursor.fetchall()]
                self.combobox_values[field_key] = (
                    values if values else [""]
                )  # Fallback to empty string if no values found
    ...
    def _create_header(self):
        ...
        self.canvas.create_rectangle(0.0, 0.0, 720.0, 72.0, fill="#800000", outline="")
    ...
    def _create_footer(self):
        ...
        self.canvas.create_rectangle(
            0.0, 648.0, 720.0, 720.0, fill="#800000", outline=""
        )
    ...
    def _create_entry_fields(self):
        ...
        for i, ((label, field, entry_type), (label_y, entry_y)) in enumerate(
            zip(self.entry_fields, row_positions), start=1
        ):
            ...
            img = tk.PhotoImage(
                file=relative_to_assets(f"entry_{13 - i}.png")
            )  # Reverse order for images
            setattr(self, f"entry_img_{13 - i}", img)

            self.canvas.create_image(x_entry + width / 2, entry_y + 22, image=img)
            ...
            if entry_type == "display":
                ...
                entry.bind(
                    "<<ComboboxSelected>>",
                    lambda e, cb=entry, f=field: self._handle_combobox_selection(
                        e, cb, f
                    ),
                )
                ...
                def date_key_handler(event):
                    ...
                    if (
                        not content or content == "YYYY/MM/DD"
                    ...
                    if event.keysym in ("BackSpace", "Delete"):
                        if not content or content.strip() == "YYYY/MM/DD":
                            ...
                            return "break"
                        if cursor_pos in [5, 8] or (
                            event.keysym == "Delete" and cursor_pos in [4, 7]
                        ):
                            return "break"
            ...
            # Place the widget
            entry.place(x=x_entry, y=entry_y, width=width, height=44.0)
    ...
    def _setup_editing_field(self):
        ...
        now_editing_frame = tk.Frame(self.root, bg="#000000", width=100, height=30)
    ...
    def _has_changes(self):
        ...
        for field, entry in self.entries.items():
            ...
            if original_idx < 0 or original_idx >= len(
                self.items_data[self.current_index]
            ):
                continue
            ...
            if field in ["[IN]", "[OUT]", "[MIN STOCK]", "[PRICE PER UNIT]"]:
                try:
                    current_num = float(
                        current_value.replace(",", "") if current_value else 0
                    )
                    original_num = float(
                        original_value.replace(",", "") if original_value else 0
                    )
                    if (
                        abs(current_num - original_num) > 0.001
    ...
    def save_changes(self, prompt=True):
        ...
        def clean_date(value):
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid date format for Last P.O: {value}. Expected format: YYYY/MM/DD"
                )
        ...
        try:
            ...
            for field in required_fields:
                if not updated_data.get(field):
                    if prompt:
                        ...
                        show_toast(self.root, f"{field_name} is required", type="error")
                    return False
            ...
            for field, value in updated_data.items():
                original_value = str(
                    self.items_data[self.current_index][self.get_field_index(field)]
                )
                new_value = str(value)
            ...
            if not changes:
                if prompt:
                    show_toast(self.root, "No changes to save", type="info")
                return False
            ...
            try:
                ...
                try:
                    ...
                    if len(self.items_data[self.current_index]) > 0 and isinstance(
                        self.items_data[self.current_index][0], int
                    ):
                ...
                if item_id is not None:
                    # Update by ID
                    queries.update_item_by_id(
                        self.db_connection, item_id, fields_to_update
                    )
                else:
                    ...
                    set_clause = ", ".join(
                        [f"[{k}] = ?" for k in fields_to_update.keys()]
                    )
                ...
                change_descriptions = []
                for field, value in updated_data.items():
                    original_value = self.items_data[self.current_index][
                    ...
                    if str(value) != str(original_value):
                        ...
                        if field == "[UNIT OF MEASURE]":
                            ...
                            action = (
                                "increased"
                                if float(str(value)) > float(str(original_value or 0))
                                else "decreased"
                            )
                ...
                if prompt:
                    show_toast(self.root, "Item updated successfully!", type="success")
                ...
                if prompt:
                    show_toast(
                        self.root, f"Failed to update database: {str(e)}", type="error"
                    )
                return False

        except ValueError as ve:
            if prompt:
                show_toast(self.root, str(ve), type="error")
            return False
    ...
    def _format_currency(self, widget):
        ...
        try:
            ...
            if current_value:  # Only show error if there's actually a value
                show_toast(self.root, "Invalid currency value", type="error")
                widget.delete(0, tk.END)
    ...
    def _add_new_value(self, field, combobox):
        ...
        x = (
            self.root.winfo_x()
            + (self.root.winfo_width() - add_window.winfo_width()) // 2
        )
        y = (
            self.root.winfo_y()
            + (self.root.winfo_height() - add_window.winfo_height()) // 2
        )
