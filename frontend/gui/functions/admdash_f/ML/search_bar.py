...
def _resolve_db_path():
    """Return the canonical ACCDB path."""
    try:
        return get_db_path()
...
def search_items(dashboard, event=None):
    ...
    try:
        ...
        print(
            f"[DEBUG] Search initiated - Current view: {current_view}, Keyword: '{keyword}'"
        )
        ...
        try:
            ...
            if suggestions and hasattr(dashboard, "suggestions_popup"):
                try:
                    ...
                    y = (
                        dashboard.search_entry.winfo_rooty()
                        + dashboard.search_entry.winfo_height()
                    )
                    dashboard.suggestions_popup.geometry(
                        f"{dashboard.search_entry.winfo_width()}x150+{x}+{y}"
                    )
            ...
            try:
                ...
                filter_query = (
                    f"SELECT {column_list} FROM [{table_name}] WHERE {where_conditions}"
                )
                ...
                try:
                    ...
                    if current_view == "Restock List":
                        ...
                        status_index = (
                            headers.index("STATUS") if "STATUS" in headers else -1
                        )
                        balance_index = (
                            headers.index("BALANCE") if "BALANCE" in headers else -1
                        )
                        min_stock_index = (
                            headers.index("MIN STOCK") if "MIN STOCK" in headers else -1
                        )
                        ...
                        for row in rows:
                            if len(row) > status_index and status_index >= 0:
                                status = (
                                    str(row[status_index]).lower()
                                    if row[status_index]
                                    else ""
                                )
                                if status and (
                                    "out of stock" in status or "low in stock" in status
                                ):
                            ...
                            if (
                                balance_index >= 0
                                and min_stock_index >= 0
                                and len(row) > max(balance_index, min_stock_index)
                            ):
                                balance = (
                                    row[balance_index]
                                    if row[balance_index] not in (None, "")
                                    else 0
                                )
                                min_stock = (
                                    row[min_stock_index]
                                    if row[min_stock_index] not in (None, "")
                                    else 0
                                )
                                try:
                ...
                if not rows:
                    ...
                    try:
                        ...
                        if current_view == "Restock List" and rows:
                            ...
                            min_stock_index = (
                                headers.index("MIN STOCK")
                                if "MIN STOCK" in headers
                                else -1
                            )
                            ...
                            for row in rows:
                                if len(row) > status_index and status_index >= 0:
                                    ...
                                    if status and (
                                        "out of stock" in status
                                        or "low in stock" in status
                                    ):
                                        filtered_rows.append(row)
                                        continue
            ...
            if dashboard.table:
                    ...
                    if current_view in ["ITEMS_LIST"]:
                        ...
                        if hasattr(dashboard, "sort_by_column"):
                            dashboard.table.heading(
                                col,
                                command=lambda _col=col: dashboard.sort_by_column(
                                    _col, False
                                ),
                            )
                    elif current_view == "Restock List":
                ...
                for i, row in enumerate(rows):
                    ...
                    if hasattr(dashboard.table, "_checked_items"):
                        ...
                        row_id = dashboard.table.insert(
                            "", tk.END, values=sanitized_row
                        )
                        # Check if this item was previously checked
                        if sanitized_row[0] in [
                            dashboard.table.item(item)["values"][0]
                ...
                if (
                    ...
                    try:
                        ...
                        if db_path_retry and os.path.exists(db_path_retry):
                            ...
                            cur2.execute(
                                f"SELECT {', '.join([f'[{c}]' for c in headers])} FROM [{table_name}] WHERE LCASE([{suggestion_field}]) = ?",
                                (keyword.lower(),),
                            )
                            retry_rows = cur2.fetchall()
                            if retry_rows:
                                print(
                                    f"[DEBUG] Retry found {len(retry_rows)} rows; updating table"
                                )
            ...
            if hasattr(dashboard, "status_label"):
                dashboard.status_label.config(
                    text=f"Error: Failed to search items: {e}", fg="#D72631"
                )
        finally:
            try:
                if "cursor" in locals():
...
def select_suggestion(dashboard, event=None):
    ...
    try:
        if (
            hasattr(dashboard, "suggestions_listbox")
            and dashboard.suggestions_listbox.curselection()
        ):
            selected = dashboard.suggestions_listbox.get(
                dashboard.suggestions_listbox.curselection()
            )
...
def show_search_bar(dashboard):
    ...
    if current_view in VIEW_CONFIGS:
        if hasattr(dashboard, "search_container"):
            dashboard.search_container.pack(
