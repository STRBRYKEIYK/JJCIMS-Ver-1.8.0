...
def forgot_password(
    parent, open_dialogs, DB_PATH, fernet, decrypt_2fa, reset_password_func
):
    ...
    tk.Label(
        top, text="Username", font=("Segoe UI", 14), bg="#000000", fg="#E8C999"
    ).pack(pady=(8, 2))
    username_entry = tk.Entry(
        ...
        bg="#23272b",
        fg="#E8C999",
        insertbackground="#E8C999",
        width=22,
        relief="solid",
    )
    username_entry.pack(pady=2)
    tk.Label(
        top, text="2FA Code", font=("Segoe UI", 14), bg="#000000", fg="#E8C999"
    ).pack(pady=(8, 2))
    code_entry = tk.Entry(
        top,
        font=("Segoe UI", 14),
    ...
    def verify():
        ...
        if not username or not code:
            messagebox.showwarning(
                "Input Error", "Please enter both username and 2FA code."
            )
            return
            ...
            if row and row[0] and row[1] in ("Level 2", "Level 3"):
                try:
                    ...
                    messagebox.showerror(
                        "2FA Error", "2FA secret could not be decrypted."
                    )
                    return
                import pyotp
                ...
                messagebox.showerror(
                    "User Not Allowed",
                    "Username not found, 2FA not set up, or insufficient access level.",
                )
        except Exception as e:
            print(f"Database error (forgot_password): {e}")
            messagebox.showerror(
                "Database Error",
                "Could not connect to the database. Please contact support.",
            )

    tk_verify_btn = tk.Button(
        top,
        text="Verify",
...
def reset_password(parent, open_dialogs, DB_PATH, fernet, username):
    ...
    def do_reset():
        ...
        try:
            ...
            reset_top.destroy()
            messagebox.showinfo(
                "Password Reset", "Your password has been reset. You may now log in."
            )
            return
        except Exception as e:
            print(f"Database error (reset_password): {e}")
            messagebox.showerror(
                "Database Error", "Could not reset password. Please contact support."
            )
        finally:
            try:
                if cursor:
