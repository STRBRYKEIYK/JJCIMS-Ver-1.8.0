...
class LoadingScreen:
    ...
    def load_images(self):
        ...
        try:
            ...
            win_logo_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'assets', 'winLogo.png'))
            self.win_logo_original = Image.open(win_logo_path).convert('RGBA')
            self.win_logo_original = self.win_logo_original.resize((500, 500), Image.Resampling.LANCZOS)
            
            # Load JJCFPIS logo - bigger size
            jjc_logo_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'assets', 'JJCFPIS.png'))
            self.jjc_logo_original = Image.open(jjc_logo_path).convert('RGBA')
            self.jjc_logo_original = self.jjc_logo_original.resize((600, 600), Image.Resampling.LANCZOS)
    ...
    def create_loading_circle(self, size=160):
        ...
        if progress_degrees > 0:
            ...
            # Draw filled arc
            mask_draw.pieslice([center - radius, center - radius, center + radius, center + radius],
            ...
            for i in range(5):
                alpha = max(0, 80 - i * 15)
                progress_draw.ellipse([center - radius - i - 2, center - radius - i - 2, 
                                     center + radius + i + 2, center + radius + i + 2], 
                                    outline=(255, 111, 0, alpha), width=1)
            
            # Main progress ring
            progress_draw.ellipse([center - radius, center - radius, center + radius, center + radius], 
                                outline=(255, 111, 0, 255), width=circle_width)
            
            # Inner progress ring
            inner_radius = radius - 12
            progress_draw.ellipse([center - inner_radius, center - inner_radius, 
                                 center + inner_radius, center + inner_radius], 
                                outline=(255, 150, 0, 180), width=3)
        ...
        scanlined_circle = self.create_scanline_interlacing(img, num_lines=35, line_opacity=0.25)
    ...
    def create_animated_text(self, text, font_size, y_offset=0):
        ...
        img = create_scanline_effect(img, num_lines=20, line_opacity=0.1, glow_amount=1.2)
    ...
    def create_ui(self):
        ...
        copyright_img = self.create_animated_text("© 2025 JJC Engineering Works and General Services | Made by KWF", 18)
        copyright_photo = ImageTk.PhotoImage(copyright_img)
        self.copyright_label = tk.Label(self.main_frame, image=copyright_photo, bg='#000000')
    ...
    def animate_logo_sequence(self):
        ...
        try:
            if self.current_logo == "windows":
                ...
                scanlined_logo = create_scanline_effect(faded_logo, num_lines=40, line_opacity=0.12, glow_amount=1.2)
        ...
        if self.animation_running and not self.window_destroyed:
            job_id = self.root.after(33, self.animate_logo_sequence)  # ~30fps for better performance
    ...
    def animate_loading_circle(self):
        ...
        try:
            ...
            rotated_circle = circle_img.rotate(-self.rotation_angle, resample=Image.Resampling.BILINEAR)
    ...
    def update_progress_display(self):
        ...
        # Continue updating
        if self.animation_running and not self.window_destroyed:
            job_id = self.root.after(200, self.update_progress_display)  # Update every 200ms for smooth feel
            self.after_jobs.append(job_id)
        ...
        def preload_windows():
            try:
                preload_steps = [
                    ...
                    ("Loading Employee Dashboard...", self.preload_employee_dashboard, 1.5),
                    ("Loading Fabrication Window...", self.preload_fabrication_window, 1.5),
                ...
                for i, (status, preload_func, step_duration) in enumerate(preload_steps):
                    if not self.animation_running:
                        break
                        
                    self.preload_status = status
                    self.preload_progress = int((current_time / target_ready_time) * 100)
