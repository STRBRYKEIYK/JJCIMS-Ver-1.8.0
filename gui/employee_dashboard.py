...
from gui.functions.emplydash_f.emplydash_utils import (
    focus_next_widget, update_clock, filter_items as util_filter_items
)
...
class MainBrowser:
    def refresh_type_buttons(self):
        ...
        try:
            ...
            cursor.execute("SELECT DISTINCT [TYPE] FROM [ITEMSDB] WHERE [TYPE] IS NOT NULL AND [TYPE] <> ''")
    ...
    def show_skeleton_screen(self, rows=8):
        ...
        self.scroll_y = ttk.Scrollbar(self.treeview_container, orient=tk.VERTICAL, style="Custom.Vertical.TScrollbar")
        self.table = ttk.Treeview(
            self.treeview_container,
            columns=("NAME", "BRAND", "TYPE", "LOCATION", "UNIT OF MEASURE", "STATUS", "BALANCE"),
            show="headings",
    ...
    def __init__(self):
        ...
        try:
            logo_path = Path(__file__).resolve().parent.parent / "assets" / "emp_dash.png"
        ...
        self.search_button = tk.Button(self.header, text="Search", command=self.search_items, bg=BUTTON_PRIMARY, fg=TEXT_PRIMARY)
        ...
        # Sidebar
        self.sidebar = tk.Frame(self.root, bg=SIDEBAR_COLOR, width=600)  # Light gray sidebar
        ...
        self.user_label.pack(side=tk.TOP, anchor="w", padx=10, pady=10)  # Adjust padding for alignment
        Separator(self.sidebar, orient="horizontal").pack(fill=tk.X, padx=10, pady=5)
        Separator(self.sidebar, orient="horizontal").pack(fill=tk.X, padx=10, pady=5)
        ...
        # Table Frame
        self.table_frame = tk.Frame(self.root, bg=BG_COLOR, width=1400)  # Dark background
        ...
        self.status_label.pack(side=tk.TOP, pady=5, padx=0, fill=tk.X, expand=True)  # Center horizontally and vertically
        ...
        self.update_timer_id = self.root.after(1000, self.update_session_timer)  # Start the timer
        ...
        try:
            cart_image_path = Path(__file__).resolve().parent.parent / "assets" / "cart_button.png"
        ...
        for col in ("NAME", "BRAND", "TYPE", "LOCATION", "UNIT OF MEASURE", "STATUS", "BALANCE"):
            self.table.heading(col, text=col, command=lambda _col=col: self.sort_by_column(_col, False))
    ...
    def load_items(self):
        ...
        if not hasattr(self, 'table') or not self.root.winfo_exists() or not self.table.winfo_exists():
            return
        ...
        def finish_loading():
            ...
            try:
                ...
                for row in rows:
                    ...
                    status = display_row[5] if len(display_row) > 5 and display_row[5] else ""
                    if status.strip().lower() == 'out of stock':
                        ...
                        self.table.tag_configure("outofstock", background="#555555", foreground="#999999")
                        self.table.item(item_id, tags=("outofstock",))
                # Auto-sort rows A-Z by NAME after inserting
                items = [(self.table.set(row_id, "NAME"), row_id) for row_id in self.table.get_children("")]
                ...
                print(f"[DEBUG] load_items: {len(rows)} rows fetched from ITEMSDB table.")
            except Exception as e:
    ...
    def search_items(self, event=None):
        ...
        try:
            ...
            if suggestions:
                ...
                self.suggestions_popup.geometry(f"{self.search_entry.winfo_width()}x150+{x}+{y}")
            ...
            cursor.execute(filter_query, (f"%{keyword.lower()}%", f"%{keyword.lower()}%", f"%{keyword.lower()}%", f"%{keyword.lower()}%", f"%{keyword.lower()}%"))
            rows = cursor.fetchall()
            ...
            self.status_label.config(
                text=f"Found {len(rows)} item(s) matching '{keyword}'", 
                fg=SUCCESS_COLOR
            )
            ...
            self.status_label.config(text=f"Error: Failed to search items: {e}", fg=ERROR_COLOR)
            self.notification_manager.show_notification(
                "Search Error", 
    ...
    def go_back_to_welcome(self):
        ...
        from gui.employee_login import WelcomeWindow  # Defer the import to avoid circular dependency
    ...
    def run(self):
        ...
        if getattr(self, 'is_closing', False) or not self.root.winfo_exists():
            return
        self.update_task = self.root.after(1000, self.update_data)  # Example of a recurring task
        self.root.mainloop()
    ...
    def update_data(self):
        ...
        if getattr(self, 'is_closing', False):
            return
        try:
            if not self.root.winfo_exists() or not hasattr(self, 'table') or not self.table.winfo_exists():
                return
        except Exception:
            return
        if not self.updates_paused:
            ...
            if not getattr(self, 'is_closing', False):
                try:
                    if self.root.winfo_exists() and hasattr(self, 'table') and self.table.winfo_exists():
        ...
        try:
            ...
            if self.time_remaining <= 0:
                ...
                if not getattr(self, 'is_closing', False) and self.root.winfo_exists():
                    self.update_timer_id = self.root.after(1000, self.update_session_timer)
        except Exception as e:
    ...
    def filter_by_type(self, item_type):
        ...
        try:
            ...
            if item_type == "All":
                ...
                query = ("SELECT [NAME], [BRAND], [TYPE], [LOCATION], [UNIT OF MEASURE], [STATUS], [BALANCE] FROM [ITEMSDB] "
            ...
            if item_type == "Out of Stock":
                ...
                self.table.tag_configure("outofstock", background="#A9A9A9", foreground="#666666")
                self.status_label.config(text="Out of Stock Items cannot be selected.", fg=TEXT_SECONDARY)
                self.cart_button.config(state=tk.DISABLED)
            else:
                self.table.configure(show="tree headings")  # Restore the checkbox column
                for row in rows:
                    display_row = tuple(value if value is not None else "" for value in row)
                    item_id = self.table.insert("", tk.END, values=display_row)
    ...
    def go_to_checkout(self):
        ...
        for item in checked:
            ...
            if status.strip().lower() == 'out of stock':
                ...
                item_data = (values[0], values[1], values[2], values[3], values[6], 1)  # (NAME, BRAND, TYPE, LOCATION, BALANCE, default quantity=1)
    ...
    def update_cart_button_state(self, event=None):
        ...
        if hasattr(self, 'status_label') and 'Out of Stock' in self.status_label.cget('text'):
            self.cart_button.config(state=tk.DISABLED)
