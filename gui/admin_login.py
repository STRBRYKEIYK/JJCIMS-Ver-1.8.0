...
from gui.functions.admdash_f.tfas.tfa_su_wizard import open_2fa_wizard_modal
...
def create_text_image(text, font_size, is_bold=False):
    ...
    img = Image.new('RGBA', (text_width + 40, text_height + 20), (0, 0, 0, 0))  # Added extra space
...
class AdminLogin:
    def show_toast(self, message, duration=None, type="error"):
        ...
        label = tk.Label(frame, text=f"{icon}  {message}", font=("Segoe UI", 14, "bold"), bg=color, fg="#fffde7", padx=20, pady=10)
        label.pack()
    ...
    def setup_ui(self):
        ...
        logo_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'assets', 'adm_1.png'))
        ...
        def on_leave(canvas, oval, color):
            canvas.itemconfig(oval, fill=color)
        # Red (close)
        red_btn = tk.Canvas(control_frame, width=22, height=22, bg=BG_COLOR, highlightthickness=0, bd=0)
        red_btn.grid(row=0, column=0, padx=(0, 4))
        red_oval = red_btn.create_oval(3, 3, 19, 19, fill="#ff5f57", outline="#e33e41", width=1)
        ...
        # Orange (minimize)
        yellow_btn = tk.Canvas(control_frame, width=22, height=22, bg=BG_COLOR, highlightthickness=0, bd=0)
        yellow_btn.grid(row=0, column=1, padx=(0, 4))
        yellow_oval = yellow_btn.create_oval(3, 3, 19, 19, fill="#FFBD2E", outline="#BF8E1E", width=1)
        yellow_btn.bind("<Button-1>", lambda e: self.root.iconify())
        yellow_btn.bind("<Enter>", lambda e: on_enter(yellow_btn, yellow_oval, "#BF8E1E"))
        yellow_btn.bind("<Leave>", lambda e: on_leave(yellow_btn, yellow_oval, "#FFBD2E"))
        # (Optional) Add tooltips for accessibility
        def add_tooltip(widget, text):
            tip = tk.Toplevel(widget)
            tip.withdraw()
            tip.overrideredirect(True)
            label = tk.Label(tip, text=text, bg="#000000", fg="#fffde7", font=("Arial", 9), padx=4, pady=2)
            label.pack()
            def show(event):
                x = event.x_root + 10
                y = event.y_root + 5
        ...
        self.username_entry = tk.Entry(right_inner, font=("Segoe UI", 18), width=30, relief="solid", bd=1, bg=ENTRY_BG, fg=FG_COLOR, insertbackground=FG_COLOR)
        ...
        self.username_error_label = tk.Label(right_inner, text="", font=("Segoe UI", 12), fg="#b71c1c", bg=RIGHT_BG)
        ...
        self.password_entry = tk.Entry(right_inner, font=("Segoe UI", 18), width=30, relief="solid", bd=1, show="*", bg=ENTRY_BG, fg=FG_COLOR, insertbackground=FG_COLOR)
        self.password_entry.pack(pady=(0, 5))
        self.password_error_label = tk.Label(right_inner, text="", font=("Segoe UI", 12), fg="#b71c1c", bg=RIGHT_BG)
        self.password_error_label.pack(pady=(0, 10))
        self.login_button = tk.Button(right_inner, text="Login", command=self.login, bg=BUTTON_BG, fg=BUTTON_FG, font=("Segoe UI", 20, "bold"), width=20, relief="flat", activebackground=ENTRY_BG, activeforeground=ACCENT_COLOR)
        ...
        setup_2fa_btn = tk.Button(right_inner, text='Setup 2FA', font=("Segoe UI", 12, 'underline'), bg=RIGHT_BG, fg=ACCENT_COLOR, relief='flat', bd=0, cursor='hand2', command=self.setup_2fa, activebackground=RIGHT_BG, activeforeground=ACCENT_COLOR)
        setup_2fa_btn.pack(pady=(0, 10))
        self.login_status_label = tk.Label(right_inner, text="", font=("Segoe UI", 14, "bold"), fg="#b71c1c", bg=RIGHT_BG)
    ...
    def setup_2fa(self):
        ...
        username = self.username_entry.get().strip() if hasattr(self, 'username_entry') else self.username
        db_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../database/JJCIMS.accdb'))
    ...
    def prompt_2fa_setup(self):
        ...
        top.geometry(f'+{x}+{y}')
        tk.Label(top, text='Looks like you don\'t have 2FA yet.', font=("Segoe UI", 16, "bold"), bg=RIGHT_BG, fg=ACCENT_COLOR).pack(pady=(30, 10))
        tk.Label(top, text='Would you like to set it up now?', font=("Segoe UI", 12), bg=RIGHT_BG, fg=FG_COLOR).pack(pady=(0, 20))
        ...
        setup_btn = tk.Button(
            btn_frame, text='Set up 2FA', font=("Segoe UI", 13, "bold"),
            bg='#f5f5f7', fg='#0071e3', activebackground='#e5e5ea', activeforeground='#0071e3',
            relief='flat', bd=0, padx=18, pady=6, cursor='hand2', command=setup_2fa_action
        )
        setup_btn.grid(row=0, column=0, padx=10)
        later_btn = tk.Button(
            btn_frame, text='Maybe, Later?', font=("Segoe UI", 13),
            bg='#f5f5f7', fg='#1d1d1f', activebackground='#e5e5ea', activeforeground='#1d1d1f',
            relief='flat', bd=0, padx=18, pady=6, cursor='hand2', command=maybe_later_action
        )
    ...
    def login(self):
        ...
        try:
            ...
            try:
                ...
                cursor = conn.cursor()
                query = ("SELECT [Access Level], [Password], [2FA Secret] FROM [emp_list] "
                ...
                try:
                    ...
                    self.show_toast("Unable to connect to the database. Please try again later.", type="error")
                    return
            ...
            if result:
                ...
                try:
                    ...
                    self.show_toast("There was a problem verifying your password. Please contact support.", type="error")
                    return
                if decrypted_pw == password:
                    self.show_toast(f"Welcome back, {username}! Login successful.", type="success")
                    if enc_2fa:
                        try:
                            secret = self.decrypt_2fa(enc_2fa)
                        except Exception as e:
                            print(f"2FA secret error: {e}")
                            self.show_toast("There was a problem with your 2FA setup. Please contact support.", type="error")
                            return
                        self.prompt_2fa(secret)
                    else:
                        self.prompt_2fa_setup()
                        self.show_toast("Login successful! Please set up 2FA for extra security.", type="success")
                else:
                    self.password_entry.delete(0, tk.END)
                    self.show_toast("Incorrect username or password. Please try again.", type="error")
            else:
                self.password_entry.delete(0, tk.END)
                self.show_toast("Incorrect username or password. Please try again.", type="error")
        except Exception:
            self.show_toast("A database error occurred. Please try again later.", type="error")
        finally:
            try:
