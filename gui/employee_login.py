...
def create_text_with_glow(text, font_size, is_bold=False):
    ...
    # Main text
    draw.text((padding, padding), text, font=font, fill=GLOW_COLOR_SOLID)  # Solid accent color
...
class WelcomeWindow:
    def show_toast(self, message, duration=None, type="error"):
        ...
        label = tk.Label(frame, text=f"{icon}  {message}", font=("Arial", 14, "bold"), bg=color, fg=TEXT_PRIMARY, padx=20, pady=10)
        label.pack()
    ...
    def __init__(self):
        ...
        configure_window(self.root, title="JJCFPIS - Dashboard", width=1024, height=768, resizable=True)
    ...
    def load_employee_names(self):
        ...
        try:
            ...
            rows = connector.fetchall('SELECT [First Name], [Last Name], [Middle Name], [Username] FROM [emp_list]')
            for row in rows:
                ...
                if first_name and last_name:
                    ...
                    if middle_name:
                        full_name_with_middle = f"{first_name} {middle_name} {last_name}".lower()
    ...
    def _add_logo(self):
        ...
        logo_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'assets', 'emp_1.png'))
        original_img = Image.open(logo_path).resize((750, 750), Image.Resampling.LANCZOS)
    ...
    def _add_control_buttons(self):
        ...
        self.control_buttons_frame = tk.Frame(self.left_panel, bg=BG_COLOR)  # Pure Black
        self.control_buttons_frame.place(relx=0.02, rely=0.02, anchor="nw")

        # macOS-style close (red) button
        close_btn = tk.Canvas(self.control_buttons_frame, width=22, height=22, bg=BG_COLOR, highlightthickness=0, bd=0)
        close_btn.grid(row=0, column=0, padx=2)
        close_oval = close_btn.create_oval(3, 3, 19, 19, fill=MACOS_RED, outline=MACOS_RED_HOVER, width=2)
        close_btn.bind("<Button-1>", lambda event: self.close_program())
        def on_close_hover(e): close_btn.itemconfig(close_oval, fill=MACOS_RED_HOVER)
        def on_close_leave(e): close_btn.itemconfig(close_oval, fill=MACOS_RED)
        close_btn.bind("<Enter>", on_close_hover)
        close_btn.bind("<Leave>", on_close_leave)

        # macOS-style minimize (yellow) button
        min_btn = tk.Canvas(self.control_buttons_frame, width=22, height=22, bg=BG_COLOR, highlightthickness=0, bd=0)
        min_btn.grid(row=0, column=1, padx=2)
        min_oval = min_btn.create_oval(3, 3, 19, 19, fill=MACOS_YELLOW, outline=MACOS_YELLOW_HOVER, width=2)
        min_btn.bind("<Button-1>", lambda event: self.hide_window())
        def on_min_hover(e): min_btn.itemconfig(min_oval, fill=MACOS_YELLOW_HOVER)
        def on_min_leave(e): min_btn.itemconfig(min_oval, fill=MACOS_YELLOW)
    ...
    def _setup_right_panel(self):
        ...
        self.right_panel_inner = tk.Frame(self.right_panel, bg=SIDEBAR_COLOR)  # Muted gray
    ...
    def _add_input_fields(self):
        ...
        name_img = create_text_with_glow("Enter Your Name, Username, or Any Part", 36, is_bold=True)
        self.label = tk.Label(
    ...
    def _on_entry_keyrelease(self, event=None):
        ...
        if typed:
            ...
            matches = [s for s in self.suggestion_list if typed in s['first'].lower() or typed in s['last'].lower() or typed in s['middle'].lower() or typed in s['username'].lower()]
            if matches:
    ...
    def _show_suggestion_popup(self, matches):
        ...
        for match in matches:
            if match['username']:
                display = f"{match['first']} {match['last']} ({match['username']})".strip()
            else:
    ...
    def update_error_label(self, event=None):
        ...
        elif name != "username" and name not in self.valid_name_parts and name not in self.valid_full_names:
            self.show_toast("Sorry, only registered employees can access this system.", type="error")
        else:
    ...
    def show_custom_prompt(self, title, message):
        ...
        prompt.title(title)
        prompt.configure(bg=SIDEBAR_COLOR, highlightbackground=TEXT_SECONDARY, highlightthickness=2)  # Add border
        ...
        # Message
        label = tk.Label(prompt, text=message, font=("Arial", 18, "bold"), bg=SIDEBAR_COLOR, fg=ACCENT_COLOR, wraplength=560, justify="center")
        ...
        def on_no():
            ...
            prompt.destroy()
        yes_btn = tk.Button(
            btn_frame, text="Yes", font=("Arial", 15, "bold"), bg=BUTTON_PRIMARY, fg=TEXT_PRIMARY,
            width=16, height=2, relief="flat", command=on_yes, activebackground=BUTTON_PRIMARY, activeforeground=TEXT_PRIMARY
        )
        yes_btn.pack(side="left", padx=30, ipadx=10, ipady=2, expand=True, fill="x")
        no_btn = tk.Button(
            btn_frame, text="No", font=("Arial", 15, "bold"), bg=ERROR_COLOR, fg=TEXT_PRIMARY,
            width=16, height=2, relief="flat", command=on_no, activebackground=ERROR_COLOR, activeforeground=TEXT_PRIMARY
        )
    ...
    def get_username_for_name(self, name):
        ...
        try:
            connector = get_connector()
            rows = connector.fetchall('SELECT [Username], [First Name], [Last Name], [Middle Name] FROM [emp_list]')
            ...
            for row in rows:
                ...
                if (input_lower == first_name.lower() or 
                    input_lower == last_name.lower() or
                    input_lower == username.lower() or
                    input_lower == f"{first_name} {last_name}".lower() or
                    (middle_name and input_lower == f"{first_name} {middle_name} {last_name}".lower()) or
                    (first_name and input_lower in first_name.lower().split())):
    ...
    def get_access_level(self, name):
        ...
        try:
            connector = get_connector()
            rows = connector.fetchall('SELECT [Access Level], [First Name], [Last Name], [Middle Name], [Username] FROM [emp_list]')
    ...
    def continue_to_main(self):
        ...
        if name_lower != "username" and name_lower not in self.valid_name_parts and name_lower not in self.valid_full_names:
            self.show_toast("Sorry, only registered employees or 'username' can access this system.", type="error")
            return
        # Check access level
    ...
    def show_dashboard_or_admin_prompt(self, message):
        ...
        prompt.configure(bg=SIDEBAR_COLOR, highlightbackground=TEXT_SECONDARY, highlightthickness=2)
        prompt.geometry("600x320")
        prompt.overrideredirect(True)
        prompt.grab_set()
        prompt.resizable(False, False)
        ...
        def to_admin():
            ...
            prompt.destroy()
        dash_btn = tk.Button(
            btn_frame, text="Employee Dashboard", font=("Arial", 15, "bold"), bg=BUTTON_PRIMARY, fg=TEXT_PRIMARY,
            width=18, height=2, relief="flat", command=to_dashboard, activebackground=BUTTON_PRIMARY, activeforeground=TEXT_PRIMARY
        )
        dash_btn.pack(side="left", padx=30, ipadx=10, ipady=2, expand=True, fill="x")
        admin_btn = tk.Button(
            btn_frame, text="Admin Login", font=("Arial", 15, "bold"), bg=ERROR_COLOR, fg=TEXT_PRIMARY,
            width=18, height=2, relief="flat", command=to_admin, activebackground=ERROR_COLOR, activeforeground=TEXT_PRIMARY
        )
    ...
    def open_admin_login(self):
        ...
        msg = ("This section is for Admins and C.T.Os only.\n\n" +
