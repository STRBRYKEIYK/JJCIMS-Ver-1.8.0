...
def update_clock(clock_label, root, update_clock_id_holder):
    ...
    update_clock_id_holder[0] = root.after(1000, lambda: update_clock(clock_label, root, update_clock_id_holder))
...
def load_items(db, table, root):
    def load_data():
        try:
            ...
            print(f"Error: {e}")
            root.after(0, lambda: messagebox.showerror("Error", f"Failed to load items: {e}"))
    ...
    def update_table(rows):
        ...
        for row in table.get_children():
            table.delete(row)
        for row in rows:
            sanitized_row = tuple(value if value is not None else "N/A" for value in row)
