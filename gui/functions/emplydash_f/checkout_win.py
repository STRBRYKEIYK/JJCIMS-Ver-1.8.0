...
class CheckoutWindow:
    def __init__(self, selected_items):
        ...
        configure_window(self.root, title="JJCIMS - Checkout", width=1024, height=768, resizable=True)
        ...
        logo_path = Path(__file__).resolve().parent.parent.parent.parent / "assets" / "checkout.png"
        try:
            logo_image = tk.PhotoImage(file=str(logo_path))
            self.logo_label = tk.Label(self.header_content, image=logo_image, bg=HEADER_COLOR)
            self.logo_label.image = logo_image  # Keep a reference to avoid garbage collection
        ...
        current_user = global_state.current_user if global_state.current_user else "Unknown User"
        ...
        # Table Headers
        headers = ["NAME", "BRAND", "TYPE", "LOCATION", "BALANCE", "   ", "QUANTITY", "   ", "   "]
        for col, header in enumerate(headers):
            label = tk.Label(self.table_frame, text=header, font=("Arial", 12, "bold"), bg=TABLE_HEADER, fg=TEXT_PRIMARY, borderwidth=1, relief="solid")
        ...
        for row, item in enumerate(self.selected_items, start=1):
            ...
            def remove_item(item_name=name):
                ...
                self.selected_items = [i for i in self.selected_items if i[0] != item_name]
                self.root.destroy()
                if not self.selected_items:
            ...
            trash_icon_path = Path(__file__).resolve().parent.parent.parent.parent / "assets" / "x.png"
            ...
            tk.Label(self.table_frame, text=name, font=("Arial", 10), bg=TABLE_BG, fg=TEXT_PRIMARY, borderwidth=1, relief="solid").grid(row=row, column=0, sticky="nsew", padx=1, pady=1)
            tk.Label(self.table_frame, text=brand, font=("Arial", 10), bg=TABLE_BG, fg=TEXT_PRIMARY, borderwidth=1, relief="solid").grid(row=row, column=1, sticky="nsew", padx=1, pady=1)
            tk.Label(self.table_frame, text=item_type, font=("Arial", 10), bg=TABLE_BG, fg=TEXT_PRIMARY, borderwidth=1, relief="solid").grid(row=row, column=2, sticky="nsew", padx=1, pady=1)
            tk.Label(self.table_frame, text=location, font=("Arial", 10), bg=TABLE_BG, fg=TEXT_PRIMARY, borderwidth=1, relief="solid").grid(row=row, column=3, sticky="nsew", padx=1, pady=1)
            tk.Label(self.table_frame, text=balance, font=("Arial", 10), bg=TABLE_BG, fg=TEXT_PRIMARY, borderwidth=1, relief="solid").grid(row=row, column=4, sticky="nsew", padx=1, pady=1)

            quantity_var = tk.IntVar(value=quantity)
            self.quantity_inputs[name] = quantity_var  # Use name as key instead of item_id

            # Load the - button image
            minus_image_path = Path(__file__).resolve().parent.parent.parent.parent / "assets" / "-.png"
            ...
            decrement_button.image = minus_image  # Keep a reference to avoid garbage collection
            decrement_button.grid(row=row, column=5, sticky="nsew", padx=1, pady=1)

            # Load the + button image
            plus_image_path = Path(__file__).resolve().parent.parent.parent.parent / "assets" / "+.png"
            ...
            increment_button = tk.Button(
                self.table_frame,
                image=plus_image,
                command=lambda qv=quantity_var, bal=balance: increment_quantity(qv, bal),
                bg=TABLE_BG,
                borderwidth=0  # Remove border for a cleaner look
            )
            increment_button.image = plus_image  # Keep a reference to avoid garbage collection
            ...
            def validate_and_snap_quantity(*args, qv=None, max_balance=None, dec_btn=None, inc_btn=None):
            ...
            entry.bind("<FocusOut>", lambda e, qv=quantity_var, mb=balance, dec_btn=decrement_button, inc_btn=increment_button: validate_and_snap_quantity(qv=qv, max_balance=mb, dec_btn=dec_btn, inc_btn=inc_btn))

            self.quantity_inputs[name] = quantity_var  # Use name as key instead of item_id

            # Attach the trace callback with the correct max_balance and buttons for this row
            quantity_var.trace_add("write", lambda *args, qv=quantity_var, mb=balance, dec_btn=decrement_button, inc_btn=increment_button: validate_and_snap_quantity(qv=qv, max_balance=mb, dec_btn=dec_btn, inc_btn=inc_btn))
        ...
        self.back_button = tk.Button(self.button_frame, text="Back", command=self.go_back, bg=BUTTON_SECONDARY, fg=TEXT_PRIMARY, font=("Arial", 12, "bold"))
        self.back_button.pack(side=tk.LEFT, padx=10)

        self.confirm_button = tk.Button(self.button_frame, text="Confirm", command=self.confirm_order, bg=SUCCESS_COLOR, fg=TEXT_PRIMARY, font=("Arial", 12, "bold"))
        ...
        try:
            ...
            for item in self.selected_items:
                ...
                try:
                    ...
                    messagebox.showerror("Input Error", f"Invalid quantity for item {name}.")
                    continue
                ...
                try:
                    ...
                    messagebox.showerror("Database Error", f"Failed to update database for item {name}: {e}")
                    continue


                # Log the action to JJCIMS.accdb in the new location
                log_file_path = Path(__file__).resolve().parent.parent.parent.parent / "database" / "JJCIMS.accdb"
            ...
            response = messagebox.askyesno("Order Again?", "Do you want to order more items?")
