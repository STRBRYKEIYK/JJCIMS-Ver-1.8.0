...
from tkinter import Tk, Canvas, Entry, Button, PhotoImage, IntVar, BooleanVar, Frame, messagebox, filedialog
...
class ImportFrame(Frame):
    def __init__(self, parent, controller):
        ...
        self.system_paths = {
            "JJCIMS.accdb": self.OUTPUT_PATH / "database" / "JJCIMS.accdb",
            "Employee List.accdb": self.OUTPUT_PATH / "database" / "JJCIMS.accdb",  # mapped to canonical DB
            "logs.xlsx": self.OUTPUT_PATH / "database" / "logs.xlsx"
        }
    ...
    def validate_file(self, file_path):
        ...
        try:
            ...
            elif canonical_key.endswith('.xlsx'):
                return self.validate_excel_file(file_path, canonical_key)
    ...
    def validate_excel_file(self, file_path, file_name):
        """Validate Excel files"""
        try:
            if file_name == "logs.xlsx":
                if not HAS_OPENPYXL or openpyxl is None:  # type: ignore[truthy-bool]
                    return False, "Excel validation requires 'openpyxl' to be installed."
                workbook = openpyxl.load_workbook(file_path)
                required_sheets = ["admin", "employees"]  # Required sheets for logs
                for sheet in required_sheets:
                    if sheet not in workbook.sheetnames:
                        workbook.close()
                        return False, f"Required sheet '{sheet}' not found in logs file."
                workbook.close()
                return True, "Valid logs Excel file."
            
            return False, "Unknown Excel file type."
    ...
    def _select_files_and_queue(self):
        ...
        try:
            ...
            file_paths = filedialog.askopenfilenames(
                title="Select files to import",
                filetypes=[("Access/Excel", "*.accdb *.xlsx")],
                initialdir=os.path.expanduser("~")
            )
    ...
    def import_file(self, source_path, file_name):
        ...
        try:
            ...
            if destination_path.exists():
                backup_path = destination_path.with_suffix(f".backup{destination_path.suffix}")
...
class ExportFrame(Frame):
    def __init__(self, parent, controller):
        ...
        def load_images():
            ...
            for i in range(5, 8):
                try:
                    self.checkbox_images[i] = {
                        'unchecked': PhotoImage(file=relative_to_assets(f"image_{i}.png")),
                        'checked': PhotoImage(file=relative_to_assets(f"image_{i}_selected.png"))
                    }
                except Exception as e:
            ...
            for i in range(2, 5):
                try:
                    self.radio_images[i] = {
                        'unselected': PhotoImage(file=relative_to_assets(f"image_{i}.png")),
                        'selected': PhotoImage(file=relative_to_assets(f"image_{i}_selected.png"))
                    }
                except Exception as e:
        ...
        def update_checkbox_image(image_id, checkbox_num, var):
            ...
            if self.checkbox_images[checkbox_num][state]:
                self.canvas.itemconfig(image_id, image=self.checkbox_images[checkbox_num][state])
        ...
        if self.radio_images[2]['selected']:
            ...
            self.image_2 = self.canvas.create_oval(318, 589, 328, 599, fill="#800000", outline="#FFFFFF")

        if self.radio_images[3]['unselected']:
            self.image_3 = self.canvas.create_image(
                653.0,
                594.0,
                image=self.radio_images[3]['unselected']
            )
        else:
            # Fallback: create a simple circle
            self.image_3 = self.canvas.create_oval(648, 589, 658, 599, fill="#191919", outline="#FFFFFF")

        if self.radio_images[4]['unselected']:
            self.image_4 = self.canvas.create_image(
                964.0,
                594.0,
                image=self.radio_images[4]['unselected']
            )
        else:
            # Fallback: create a simple circle
            self.image_4 = self.canvas.create_oval(959, 589, 969, 599, fill="#191919", outline="#FFFFFF")

        def update_radio_images():
            selected = self.export_format.get()
            for i, img_id in [(2, self.image_2), (3, self.image_3), (4, self.image_4)]:
                if self.radio_images[i]['selected'] and self.radio_images[i]['unselected']:
                    state = 'selected' if selected == (i-1) else 'unselected'
                    self.canvas.itemconfig(img_id, image=self.radio_images[i][state])

        self.canvas.tag_bind(self.image_2, '<Button-1>', 
            lambda e: (self.export_format.set(1), update_radio_images()))

        self.canvas.tag_bind(self.image_3, '<Button-1>', 
            lambda e: (self.export_format.set(2), update_radio_images()))

        self.canvas.tag_bind(self.image_4, '<Button-1>', 
        ...
        if self.checkbox_images[5]['unchecked']:
            self.image_5 = self.canvas.create_image(
                255.0,
                308.0,
                image=self.checkbox_images[5]['unchecked']
            )
        else:
            # Fallback: create a simple rectangle
            self.image_5 = self.canvas.create_rectangle(250, 303, 260, 313, fill="#191919", outline="#FFFFFF")

        self.canvas.tag_bind(self.image_5, '<Button-1>', 
        ...
        if self.checkbox_images[6]['unchecked']:
            self.image_6 = self.canvas.create_image(
                640.0,
                308.0,
                image=self.checkbox_images[6]['unchecked']
            )
        else:
            # Fallback: create a simple rectangle
            self.image_6 = self.canvas.create_rectangle(635, 303, 645, 313, fill="#191919", outline="#FFFFFF")

        self.canvas.tag_bind(self.image_6, '<Button-1>', 
        ...
        if self.checkbox_images[7]['unchecked']:
            self.image_7 = self.canvas.create_image(
                944.0,
                308.0,
                image=self.checkbox_images[7]['unchecked']
            )
        else:
            # Fallback: create a simple rectangle
            self.image_7 = self.canvas.create_rectangle(939, 303, 949, 313, fill="#191919", outline="#FFFFFF")

        self.canvas.tag_bind(self.image_7, '<Button-1>', 
            lambda e: toggle_checkbox(self.employee_var, self.image_7, 7))

    # ---------------- Export logic wiring ----------------
    def _choose_export_folder(self):
        folder = filedialog.askdirectory(title="Select export destination folder", initialdir=os.path.expanduser("~"))
        if folder:
    ...
    def _do_export(self):
        ...
        if not os.path.isdir(dest):
            try:
                ...
                messagebox.showerror("Destination Error", f"Cannot create destination folder:\n{e}")
                return
        ...
        if not any(targets.values()):
            messagebox.showwarning("No Selection", "Select at least one file to export.")
            return

        fmt = self.export_format.get()  # 1=Access,2=Excel,3=CSV
        base_dir = Path(__file__).parent.parent.parent.parent
        src_db = base_dir / 'database' / 'JJCIMS.accdb'
        # Employee list is stored in the canonical DB
        src_emp = src_db
        src_logs = base_dir / 'database' / 'logs.xlsx'
        ...
        try:
            if fmt == 1:  # Access
                ...
                if targets['logs']:
                    errors.append("Exporting logs.xlsx to Access format is not supported.")

            elif fmt == 2:  # Excel
                if targets['items_db']:
                    try:
                        self._export_table_to_excel(src_db, 'ITEMSDB', os.path.join(dest, 'ITEMSDB.xlsx'))
                        exported.append('ITEMSDB.xlsx')
                    except Exception as e:
                        errors.append(f"ITEMSDB.xlsx: {e}")
                if targets['employee']:
                    try:
                        self._export_table_to_excel(src_emp, 'Emp_list', os.path.join(dest, 'Employee_List.xlsx'))
                ...
                if targets['logs']:
                    if src_logs.exists():
                        try:
                            safe_copy(src_logs, 'logs.xlsx')
                        except Exception as e:
                            errors.append(f"logs.xlsx: {e}")
                    else:
                        errors.append("logs.xlsx not found")

            elif fmt == 3:  # CSV
                if targets['items_db']:
                    try:
                        self._export_table_to_csv(src_db, 'ITEMSDB', os.path.join(dest, 'ITEMSDB.csv'))
                        exported.append('ITEMSDB.csv')
                    except Exception as e:
                        errors.append(f"ITEMSDB.csv: {e}")
                if targets['employee']:
                    try:
                        self._export_table_to_csv(src_emp, 'Emp_list', os.path.join(dest, 'Employee_List.csv'))
                        exported.append('Employee_List.csv')
                    except Exception as e:
                        errors.append(f"Employee_List.csv: {e}")
                if targets['logs']:
                    try:
                        created = self._export_logs_to_csv(src_logs, dest)
                        exported.extend(created)
                    except Exception as e:
                        errors.append(f"logs.xlsx->CSV: {e}")
        ...
        if exported and not errors:
            ...
            messagebox.showwarning("Export Completed with Errors", f"Exported: {', '.join(exported)}\nErrors:\n- " + "\n- ".join(errors))
        else:
            messagebox.showerror("Export Failed", "\n".join(errors) if errors else "Nothing exported.")

    def _export_table_to_csv(self, accdb_path: Path, table: str, csv_path: str):
        if not HAS_PYODBC or pyodbc is None:  # type: ignore[truthy-bool]
            raise RuntimeError("CSV export from Access requires 'pyodbc' to be installed.")
    ...
    def _export_table_to_excel(self, accdb_path: Path, table: str, xlsx_path: str):
        if not HAS_PYODBC or pyodbc is None:  # type: ignore[truthy-bool]
            raise RuntimeError("Excel export from Access requires 'pyodbc' to be installed.")
        if not HAS_OPENPYXL or openpyxl is None:  # type: ignore[truthy-bool]
    ...
    def _export_logs_to_csv(self, logs_path: Path, dest_dir: str):
        if not HAS_OPENPYXL or openpyxl is None:  # type: ignore[truthy-bool]
            raise RuntimeError("Converting logs.xlsx to CSV requires 'openpyxl' to be installed.")
        if not Path(logs_path).exists():
            raise FileNotFoundError("logs.xlsx not found")
        wb = openpyxl.load_workbook(logs_path)
        created = []
        for sheet_name in ("admin", "employees"):
            if sheet_name in wb.sheetnames:
                ws = wb[sheet_name]
                out_path = os.path.join(dest_dir, f"{sheet_name}.csv")
                with open(out_path, 'w', newline='', encoding='utf-8') as f:
                    writer = csv.writer(f)
                    for row in ws.iter_rows(values_only=True):
                        writer.writerow(["" if v is None else v for v in row])
                created.append(f"{sheet_name}.csv")
        wb.close()
        if not created:
            raise RuntimeError("No expected sheets ('admin', 'employees') found in logs.xlsx")
        return created
