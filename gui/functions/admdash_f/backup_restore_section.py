...
class BackupRestoreSection(Frame):
    def __init__(self, parent, assets_backup, assets_restore, *args, **kwargs):
        ...
        import os
        self.status_file = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../config/backup_restore_status.json'))
    ...
    def show_backup(self):
        ...
        if self.backup_canvas:
            ...
            return
        c = Canvas(self, bg="#000000", width=self.width, height=self.height, bd=0, highlightthickness=0, relief="ridge")
        c.place(x=0, y=0)
        ...
        self.last_backup_text_id = c.create_text(
            785.0, 93.0, anchor="nw",
            text=self.get_last_backup_text(),
            fill="#000000", font=("Inter MediumItalic", 24 * -1)
        )
        ...
        Button(c, image=btn_img1, background="#000D1D", activebackground="#000D1D", borderwidth=0, highlightthickness=0, command=self.select_backup_folder, relief="flat").place(x=1056.0, y=558.0, width=74.93187713623047, height=57.44150161743164)
        ...
        self.entry_backup.insert(0, self.folder_path_backup or "Select backup folder...")
        
        # Button 2 - Backup Now button
        btn_img2 = PhotoImage(file=self.relative_to_assets_backup("button_2.png"))
        Button(c, image=btn_img2, background="#000D1D", activebackground="#000D1D", borderwidth=0, highlightthickness=0, command=self.backup_now, relief="flat").place(x=542.0, y=654.0, width=315.22344970703125, height=58.406524658203125)
        
        # Button 3 - Switch to Restore tab
        btn_img3 = PhotoImage(file=self.relative_to_assets_backup("button_3.png"))
        Button(c, image=btn_img3, background="#000000", activebackground="#000000", borderwidth=0, highlightthickness=0, command=self.show_restore, relief="flat").place(x=375.79473876953125, y=80.0, width=209.80926513671875, height=48.045494079589844)
        ...
        self._backup_images = [img1, img2, img3, img4, entry_img, btn_img1, btn_img2, btn_img3]
        # Update text in case it changed
        if hasattr(self, 'last_backup_text_id'):
            self.backup_canvas.itemconfig(self.last_backup_text_id, text=self.get_last_backup_text())
    ...
    def backup_now(self):
        ...
        try:
            ...
            src_db = os.path.join(base_dir, 'database', 'JJCIMS.accdb')
            ...
            if hasattr(self, 'backup_canvas') and hasattr(self, 'last_backup_text_id'):
                self.backup_canvas.itemconfig(self.last_backup_text_id, text=f"Last Backup: {self.last_backup_time}")
            ...
            self.after(1200, lambda: self.show_toast(f"Saved in: {short_path}", success=True, duration=1800))
        except Exception as e:
    ...
    def show_toast(self, message, success=True, duration=2500):
        ...
        label = tk.Label(frame, text=message, bg=bg, fg=fg, font=("Segoe UI", 13, "bold"), anchor="w", padx=18)
    ...
    def show_restore(self):
        ...
        Button(c, image=btn_img1, background="#1D0000", activebackground="#1D0000", borderwidth=0, highlightthickness=0, command=self.select_restore_folder, relief="flat").place(x=1056.0, y=558.0, width=75.0, height=57.30337142944336)
        ...
        self.entry_restore.insert(0, self.folder_path_restore or "Select restore folder...")
        
        # Button 2 - Restore Now button
        btn_img2 = PhotoImage(file=self.relative_to_assets_restore("button_2.png"))
        Button(c, image=btn_img2, background="#1D0000", activebackground="#1D0000", borderwidth=0, highlightthickness=0, command=self.restore_now, relief="flat").place(x=542.0, y=651.0, width=315.5105285644531, height=58.26612854003906)
        ...
        Button(c, image=btn_img3, background="#000000", activebackground="#000000", borderwidth=0, highlightthickness=0, command=self.show_backup, relief="flat").place(x=164.57522583007812, y=81.0, width=210.0545654296875, height=47.98387145996094)
        ...
        self._restore_images = [img1, img2, img3, img4, entry_img, btn_img1, btn_img2, btn_img3]
        # Update text in case it changed
        if hasattr(self, 'last_restore_text_id'):
            self.restore_canvas.itemconfig(self.last_restore_text_id, text=self.get_last_restore_text())
    ...
    def restore_now(self):
        ...
        try:
            ...
            dest_db = os.path.join(base_dir, 'database', 'JJCIMS.accdb')
            if os.path.exists(src_db):
                shutil.copy2(src_db, dest_db)
            else:
                self.show_toast(f"File not found: {os.path.basename(src_db)}", success=False)
                return
            ...
            if now:
                ...
                self._save_status()
                if hasattr(self, 'restore_canvas') and hasattr(self, 'last_restore_text_id'):
                    self.restore_canvas.itemconfig(self.last_restore_text_id, text=f"Last Restore: {self.last_restore_time}")
            self.show_toast("Restore is Successful!", success=True)
            short_path = os.path.basename(folder)
            self.after(1200, lambda: self.show_toast(f"Restored from: {short_path}", success=True, duration=1800))
        except Exception as e:
