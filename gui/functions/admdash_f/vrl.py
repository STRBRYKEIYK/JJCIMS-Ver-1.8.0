...
def load_restock_list(access_db_path: str | None = None, treeview=None):
    ...
    try:
        ...
        widths = {
            "NAME": 180, "BRAND": 120, "TYPE": 120, "LOCATION": 120,
            "UNIT OF MEASURE": 120, "STATUS": 100, "IN": 80, "OUT": 80,
            "BALANCE": 80, "MIN STOCK": 90, "DEFICIT": 80, "PRICE PER UNIT": 120,
            "COST": 120, "LAST PO": 120, "SUPPLIER": 150
        }
        for col in columns:
            treeview.heading(col, text=col, anchor="center", command=lambda c=col: sort_restock_column(treeview, c))
            anchor = (
                "center" if col in {"IN","OUT","BALANCE","MIN STOCK","DEFICIT","STATUS"}
                else ("e" if col in {"PRICE PER UNIT","COST"} else "w")
            )
            treeview.column(col, width=widths.get(col, 100), anchor=anchor, minwidth=60, stretch=False)

        treeview.bind("<Button-3>", lambda e: auto_resize_column_on_right_click(e, treeview))
            ...
            if isinstance(value, str):
                try:
                    ...
                    if '/' in value:
                        ...
                        formatted.append(value.split()[0])  # Take only date part if there's time
                    else:
                        ...
                        try:
                            ...
                            try:
                                ...
                                formatted.append(value.split()[0])  # Just remove time if present
    ...
    try:
        ...
        for other_col in treeview["columns"]:
            if other_col != col:
                ...
                treeview.heading(other_col, text=other_header_text,
        ...
        treeview.heading(col, text=f"{current_header_text}{indicator}",
                        command=lambda c=col: sort_restock_column(treeview, c))
        
        print(f"[DEBUG] Sorted restock list by {col} ({'descending' if reverse else 'ascending'})")
...
def preserve_column_widths(treeview):
    ...
    try:
        ...
        if not treeview or not hasattr(treeview, 'winfo_exists') or not treeview.winfo_exists():
