...
def load_fernet_key(fernet_key_path):
    ...
    if os.path.exists(fernet_key_path):
        if fernet_key_path.endswith('.py'):
            ...
            spec = importlib.util.spec_from_file_location("fernet_key_module", fernet_key_path)
...
class MultiStep2FAWizardforsett:
    def __init__(self, parent=None, username=None, db_path=None, fernet_key_path=None, popup=False, colors=None):
        # Store params
        self.parent = parent
    ...
    def show_step(self, step):
        ...
        elif step == 4 and self.breadcrumb_progression and self.breadcrumb_progression[-1] == 5:
            # If coming from step 5 to 4, show 1 -> 5 -> 4
            if self.breadcrumb_progression != [1, 5, 4]:
                self.breadcrumb_progression.append(4)
        else:
            # Normal progression
            if not self.breadcrumb_progression or self.breadcrumb_progression[-1] != step:
    ...
    def draw_breadcrumbs(self):
        ...
        for idx, step in enumerate(self.breadcrumb_progression):
            ...
            _text_id = self.canvas.create_text(x_start + idx * spacing, y, anchor="nw", text=name, fill=fill, font=("Segoe UI", 16, "bold" if step == self.current_step else "normal"), tags=(tag,))
            self._breadcrumb_tags.append((tag, step, idx))
            # Only allow going back, not forward
            if step != self.current_step and idx < self.breadcrumb_progression.index(self.current_step):
                def make_callback(target_step, target_idx):
                    def cb(event):
                        # Trim breadcrumb progression to the clicked step
                        self.breadcrumb_progression = self.breadcrumb_progression[:target_idx+1]
            ...
            if idx < len(self.breadcrumb_progression) - 1:
                self.canvas.create_text(x_start + (idx + 1) * spacing - 50, y, anchor="nw", text="→", fill="#FFFFFF", font=("Segoe UI", 16))
    ...
    def step1(self):
        ...
        self.canvas.create_rectangle(0.0, 748.0, 1400.0, 841.0, fill="#800000", outline="")
        ...
        # Username label
        self.canvas.create_text(254.0, 331.0, anchor="nw", text="Username:", fill="#FFFFFF", font=("Inter Light", 36))
        
        # Entry field
        entry_image_1 = self.safe_load_image(step, "entry_1.png")
        _entry_bg_1 = self.canvas.create_image(699.5, 468.5, image=entry_image_1)  # unused ID
        entry_1 = Entry(self.root, bd=0, bg="#000000", fg="#FFFFFF", font=("Inter Light", 30), 
        ...
        button_generate = Button(self.root, image=button_image_1, borderwidth=0, highlightthickness=0, 
                               bg="#000000", activebackground="#000000", command=None, relief="flat", state="disabled")
        button_generate.place(x=277.0, y=617.0, width=220.17446899414062, height=65.53191375732422)
        
        # Key Setup button (button_2)  
        button_image_2 = self.safe_load_image(step, "button_2.png")
        key_setup = Button(self.root, image=button_image_2, borderwidth=0, highlightthickness=0, 
                          bg="#000000", activebackground="#000000", command=None, relief="flat")
        key_setup.place(x=900.0, y=617.0, width=222.48085021972656, height=65.6907958984375)
        
        # Footer button (button_3)
        button_image_3 = self.safe_load_image(step, "button_3.png")
        footer_button = Button(self.root, image=button_image_3, borderwidth=0, highlightthickness=0, 
                              bg="#800000", activebackground="#800000", command=lambda: self.show_step(5), relief="flat")
        ...
        if not button_image_3 or button_image_3.width() == 1:  # Check if it's our fallback 1x1 image
            print("Warning: Could not load button_3.png for step 1, using text fallback")
            footer_button.destroy()  # Remove the image button
            footer_button = Button(self.root, text="Setup 2FA", font=("Segoe UI", 12, "bold"),
                                  bg="#800000", fg="#FFFFFF", activebackground="#A00000", 
                                  borderwidth=0, highlightthickness=0, command=lambda: self.show_step(5), relief="flat")
            footer_button.place(x=471.0, y=774.0, width=457.0, height=41.0)
        ...
        def toast(msg, color="#222", fg="#fff", duration=2000):
            ...
            label = tkinter.Label(toast_win, text=msg, bg=color, fg=fg, font=("Segoe UI", 12, "bold"))
        ...
        def check_username():
            ...
            try:
                ...
                if not self.db_path or not os.path.exists(self.db_path):
                    ...
                    toast("Database not found. Please check configuration.", color="#800000")
                    button_generate.config(state="disabled")
                    return
                ...
                if "IM002" in error_msg:
                    toast("MS Access driver not found. Check database configuration.", color="#800000")
                else:
            ...
            try:
                ...
                if row:
                    ...
                    if access_level in ("Level 2", "Level 3"):
                        ...
                        toast(f"Access Level must be Level 2 or 3! Current level: {access_level}", color="#800000")
                else:
                    toast("Username not found!", color="#800000")
            except pyodbc.Error as db_err:
        ...
        self._refs = [image_1, entry_image_1, button_image_1, button_image_2, 
                     entry_1, button_generate, key_setup, footer_button]
    ...
    def step2(self):
        ...
        def copy_manual_key(event=None):
            ...
            label = tkinter.Label(toast_win, text="Copied!", bg="#222", fg="#fff", font=("Segoe UI", 10, "bold"))
            label.pack(fill="both", expand=True)
        ...
        def on_enter(event):
            ...
            self.canvas.itemconfig(key_text_id, fill="#5dade2")  # Highlight the key text instead
            
        def on_leave(event):
            self.canvas.config(cursor="")
            self.canvas.itemconfig(key_text_id, fill="#FFFFFF")  # Reset to original color
        ...
        # Continue button
        button_image_1 = self.safe_load_image(step, "button_1.png")
        continue_button = Button(self.root, image=button_image_1, borderwidth=0, bg="#000000", activebackground="#000000", 
                               highlightthickness=0, command=lambda: self.show_step(3), relief="flat")
        continue_button.place(x=592.6397705078125, y=671.54931640625, width=232.5906982421875, height=63.383365631103516)
        
        # Footer button
        button_image_2 = self.safe_load_image(step, "button_2.png")
        footer_button = Button(self.root, image=button_image_2, borderwidth=0, bg="#800000", activebackground="#800000", 
                              highlightthickness=0, command=lambda: self.show_step(5), relief="flat")
        footer_button.place(x=471.0, y=774.0, width=457.0, height=41.0)
    ...
    def step3(self):
        ...
        self.canvas.create_text(356.0, 359.0, anchor="nw", text="Enter 6 digit code:", 
                               fill="#FFFFFF", font=("Inter Light", 36))
        
        # Entry fields (6) - using the new layout
        entry_images = [self.safe_load_image(step, f"entry_{i+1}.png") for i in range(6)]
        entry_positions = [
        ...
        for i, (img_x, img_y, entry_x, entry_y, width, height) in enumerate(entry_positions):
            # Create image background
            entry_bg = self.canvas.create_image(img_x, img_y, image=entry_images[i])
            entry_bgs.append(entry_bg)
            
            # Create entry field
            entry = Entry(self.root, bd=0, bg="#000000", fg="#FFFFFF", 
                         font=("Inter Light", 30), justify="center", 
                         insertbackground="#FFFFFF", highlightthickness=0)
        ...
        entry_fields[0].bind('<KeyPress>', lambda event: validate_and_move(event, entry_fields[0], entry_fields[1]))
        entry_fields[1].bind('<KeyPress>', lambda event: validate_and_move(event, entry_fields[1], entry_fields[2]))
        entry_fields[2].bind('<KeyPress>', lambda event: validate_and_move(event, entry_fields[2], entry_fields[3]))
        entry_fields[3].bind('<KeyPress>', lambda event: validate_and_move(event, entry_fields[3], entry_fields[4]))
        entry_fields[4].bind('<KeyPress>', lambda event: validate_and_move(event, entry_fields[4], entry_fields[5]))
        entry_fields[5].bind('<KeyPress>', lambda event: validate_and_move(event, entry_fields[5]))
        ...
        def on_verify():
            ...
            if totp.verify(code):
                ...
                try:
                    ...
                    conn = pyodbc.connect(r'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=' + self.db_path)
                    cursor = conn.cursor()
                    cursor.execute("UPDATE [Emp_list] SET [2FA Secret]=? WHERE username=?", (encrypted_secret, self.username))
                    conn.commit()
                    conn.close()
                    toast("2FA setup complete!", color="#228B22")
                    self.show_step(4)
                except Exception as e:
                    print(f"[DEBUG][step3] Exception during DB update: {e}", file=sys.stderr)
        ...
        # Verify button
        button_image_1 = self.safe_load_image(step, "button_1.png")
        verify_button = Button(self.root, image=button_image_1, borderwidth=0, highlightthickness=0, 
                              bg="#000000", activebackground="#000000", command=on_verify, relief="flat")
        verify_button.place(x=585.0, y=628.0, width=230.94189453125, height=62.934051513671875)
        
        # Footer button
        button_image_2 = self.safe_load_image(step, "button_2.png")
        footer_button = Button(self.root, image=button_image_2, borderwidth=0, highlightthickness=0, 
                              bg="#800000", activebackground="#800000", command=lambda: self.show_step(5), relief="flat")
        footer_button.place(x=471.0, y=774.0, width=457.0, height=41.0)
        
        self._refs = [image_1, *entry_images, button_image_1, button_image_2, *entry_fields]
    ...
    def step4(self):
        ...
        # Countdown text
        self._countdown = 5
        self._countdown_text_id = self.canvas.create_text(548.0, 523.0, anchor="nw", 
                                                         text="Going Back in ...", 
                                                         fill="#FFFFFF", font=("Inter Light", 36))
        ...
        def update_countdown():
            ...
            if self._countdown > 0:
                self.canvas.itemconfig(self._countdown_text_id, text=f"Going Back in ... {self._countdown}")
        ...
        # Footer button
        button_image_1 = self.safe_load_image(step, "button_1.png")
        footer_button = Button(self.root, image=button_image_1, borderwidth=0, highlightthickness=0, 
                              bg="#800000", activebackground="#800000", command=lambda: self.show_step(1), relief="flat")
        footer_button.place(x=471.0, y=774.0, width=457.0, height=41.0)
    ...
    def step5(self):
        ...
        # Label
        self.canvas.create_text(325.0, 354.0, anchor="nw", text="Your 2FA key:", 
                               fill="#FFFFFF", font=("Inter Light", 36))
        
        # Entry field
        entry_image_1 = self.safe_load_image(step, "entry_1.png")
        _entry_bg_1 = self.canvas.create_image(700.0, 473.0, image=entry_image_1)
        entry_1 = Entry(self.root, bd=0, bg="#000000", fg="#FFFFFF", insertbackground="#FFFFFF",
                        font=("Inter Light", 30), highlightthickness=0,
        ...
        if not self.username or not str(self.username).strip():
            toast("No username provided. Please enter your username first.", color="#800000")
            entry_1.delete(0, 'end')
            entry_1.insert(0, "Username required.")
            entry_1.config(state="disabled")
            # Disable submit button
            button_image_1 = self.safe_load_image(step, "button_1.png")
            submit_button = Button(self.root, image=button_image_1, borderwidth=0, highlightthickness=0, 
                                  bg="#000000", activebackground="#000000", command=None, relief="flat", state="disabled")
        ...
        try:
            conn = pyodbc.connect(r'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=' + self.db_path)
            cursor = conn.cursor()
            cursor.execute("SELECT [2FA Secret] FROM [Emp_list] WHERE username=?", (self.username,))
            row = cursor.fetchone()
            conn.close()
...
if __name__ == "__main__":
    ...
    MultiStep2FAWizardforsett(
        parent=root,
        username=None,
        db_path=str(WORKSPACE_ROOT / "database" / "JJCIMS.accdb"),
...
def create_2fa_setup_frame(parent, username, db_path=None, fernet_key_path=None, colors=None):
    """
