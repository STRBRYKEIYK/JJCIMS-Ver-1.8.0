...
class MultiStep2FAWizard:
    def __init__(self, parent=None, username=None, db_path=None, fernet_key_path=None, popup=True, colors=None):
        # Store params
        self.parent = parent
    ...
    def draw_breadcrumbs(self):
        ...
        for idx, step in enumerate(self.breadcrumb_progression):
            ...
            self.canvas.create_text(x_start + idx * spacing, y, anchor="nw", text=name, fill=fill, font=("Segoe UI", 14, "bold" if step == self.current_step else "normal"), tags=(tag,))
            ...
            if idx < len(self.breadcrumb_progression) - 1:
                self.canvas.create_text(x_start + (idx + 1) * spacing - 30, y, anchor="nw", text="→", fill="#FFFFFF", font=("Segoe UI", 14))

    def step1(self):
        import tkinter.messagebox
        step = 1
        # Backgrounds
        self.canvas.create_rectangle(0, 0, 700, 70, fill=self.colors["header"], outline="")
        self.canvas.create_rectangle(0, 630, 700, 700, fill=self.colors["header"], outline="")
        # Images
        ...
        entry_1 = Entry(self.root, bd=0, bg="#000000", fg="#FFFFFF", highlightthickness=0, font=("Segoe UI", 20), insertbackground="#FFFFFF")
        entry_1.place(x=155.0, y=397.0, width=389.0, height=50.0)
        if self.username:
            entry_1.insert(0, self.username)
        self.canvas.create_text(141, 328, anchor="nw", text="Username:", fill="#FFFFFF", font=("Inter Light", -20))

        # Main action buttons
        button_image_generate = PhotoImage(file=relative_to_assets(step, "button_4.png"))
        button_generate = Button(self.root, image=button_image_generate, borderwidth=0, highlightthickness=0, bg="#000000", activebackground="#000000", command=None, relief="flat", state="disabled")
        ...
        button_cancel = Button(self.root, image=button_image_cancel, borderwidth=0, bg="#000000", activebackground="#000000", highlightthickness=0, command=cancel_action, relief="flat")
        button_cancel.place(x=391.0, y=496.0, width=135.8, height=40.1)
        button_image_key_setup = PhotoImage(file=relative_to_assets(step, "button_6.png"))
        def go_to_step5():
            ...
            self.username = username
            self.show_step(5)
        key_setup = Button(self.root, image=button_image_key_setup, borderwidth=0, bg="#800000", activebackground="#800000", highlightthickness=0, command=go_to_step5, relief="flat")
        ...
        def check_username():
            ...
            try:
                ...
                conn = connector.connect()
                cursor = conn.cursor()
                cursor.execute("SELECT username FROM [emp_list] WHERE username=?", (username,))
                row = cursor.fetchone()
        ...
        def on_generate():
            ...
            try:
                ...
                conn = connector.connect()
                cursor = conn.cursor()
                cursor.execute("SELECT [Access Level], [2FA Secret] FROM [emp_list] WHERE username=?", (username,))
                row = cursor.fetchone()
                cursor.close()
                conn.close()
        ...
        # Keep references
        self._refs = [image_2, image_3, entry_image_1, button_image_generate, button_image_cancel, button_image_key_setup, entry_1, button_generate, key_setup]
    ...
    def step2(self):
        ...
        manual_key_y = 460
        self.canvas.create_text(350, manual_key_y, anchor="n", text=manual_key_text, fill="#00BFFF", font=font_tuple, tags=("manual_key",))
        ...
        self.canvas.tag_bind("manual_key", "<Enter>", lambda e: self.canvas.config(cursor="hand2"))
        self.canvas.tag_bind("manual_key", "<Leave>", lambda e: self.canvas.config(cursor=""))

        # Navigation buttons
        button_image_continue = PhotoImage(file=relative_to_assets(step, "button_5.png"))
        continue_button = Button(self.root, image=button_image_continue, borderwidth=0, bg="#000000", activebackground="#000000", highlightthickness=0, command=lambda: self.show_step(3), relief="flat")
        continue_button.place(x=278.0, y=565.0, width=143.69, height=39.16)

        button_image_key_setup = PhotoImage(file=relative_to_assets(step, "button_1.png"))
        key_setup = Button(self.root, image=button_image_key_setup, borderwidth=0, bg="#800000", activebackground="#800000", highlightthickness=0, command=lambda: self.show_step(5), relief="flat")
        key_setup.place(x=237.0, y=644.0, width=224.0, height=41.0)

        # Keep references to images/widgets
        self._refs = [image_2, image_3, qr_photo, button_image_continue, button_image_key_setup]
    ...
    def step3(self):
        ...
        self.canvas.create_text(131, 338, anchor="nw", text="Enter 6 digit code:", fill="#FFFFFF", font=("Inter Light", -20))
        # Entry fields (6)
        entry_images = [PhotoImage(file=relative_to_assets(step, f"entry_{i+1}.png")) for i in range(6)]
        ...
        for i in range(6):
            entry = Entry(self.root, bd=0, bg="#000000", fg="#FFFFFF", highlightthickness=0, insertbackground="#FFFFFF", font=("Segoe UI", 24), justify="center", show="\u2022")
        ...
        verify_button = Button(self.root, image=button_image_verify, borderwidth=0, highlightthickness=0, bg="#000000", activebackground="#000000", command=on_verify, relief="flat")
        verify_button.place(x=278.0, y=498.0, width=143.69, height=39.16)
        button_image_key_setup = PhotoImage(file=relative_to_assets(step, "button_1.png"))
        key_setup = Button(self.root, image=button_image_key_setup, borderwidth=0, highlightthickness=0, bg="#800000", activebackground="#800000", command=lambda: self.show_step(5), relief="flat")
        key_setup.place(x=237.0, y=644.0, width=226.0, height=41.0)
        self._refs = [image_2, image_3, *entry_images, button_image_verify, button_image_key_setup]
    ...
    def step4(self):
        ...
        # Countdown logic
        self._countdown = 5
        self._countdown_text_id = self.canvas.create_text(180, 362, anchor="nw", text=f"Going back to Generate page in ... {self._countdown}", fill="#FFFFFF", font=("Inter Light", -20))
        def update_countdown():
            self._countdown -= 1
            if self._countdown > 0:
                self.canvas.itemconfig(self._countdown_text_id, text=f"Going back to Generate page in ... {self._countdown}")
        ...
        def close_window():
            if self.popup:
                ...
                if hasattr(self, '_modal_parent') and self._modal_parent:
                    ...
                    self.root.pack_forget()
        return_button = Button(self.root, image=button_image_return, borderwidth=0, highlightthickness=0, bg="#000000", activebackground="#000000", command=close_window, relief="flat")
    ...
    def step5(self):
        ...
        entry_1 = Entry(self.root, bd=0, bg="#000000", fg="#ffffff", highlightthickness=0, font=("Segoe UI", 20),
                        disabledbackground="#000000", disabledforeground="#ffffff", insertbackground="#ffffff")
        entry_1.place(x=104.0, y=350.0, width=491.0, height=58.0)
        self.canvas.create_text(87, 291, anchor="nw", text="Enter your 2FA key to import:", fill="#FFFFFF", font=("Inter Light", -24))
        ...
        if not self.username or not str(self.username).strip():
            ...
            submit_button = Button(self.root, image=button_image_submit, borderwidth=0, highlightthickness=0, bg="#000000", activebackground="#000000", command=None, relief="flat", state="disabled")
        ...
        button_image_submit = PhotoImage(file=relative_to_assets(step, "button_5.png"))
        submit_button = Button(self.root, image=button_image_submit, borderwidth=0, highlightthickness=0, bg="#000000", activebackground="#000000", command=None, relief="flat")
        submit_button.place(x=281.0, y=459.0, width=138.6, height=51.77)
...
def launch_2fa_wizard(parent=None, username=None, db_path=None, fernet_key_path=None, colors=None):
    overlay = None
    try:
        if parent:
...
def open_2fa_wizard_modal(parent=None, username=None, db_path=None, fernet_key_path=None, colors=None):
    """Compatibility wrapper for older imports that used open_2fa_wizard_modal.
    Delegates to launch_2fa_wizard.
    """
    return launch_2fa_wizard(parent=parent, username=username, db_path=db_path, fernet_key_path=fernet_key_path, colors=colors)
