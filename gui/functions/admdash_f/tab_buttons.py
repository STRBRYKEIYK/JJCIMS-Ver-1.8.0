...
from utils.font_utils import get_bold_font
import pyodbc
...
def execute_access_queries():
    ...
    try:
        # Get database path
        db_path = str(Path(__file__).resolve().parent.parent.parent.parent / "database" / "JJCIMS.accdb")
...
def create_frame_outline(frame_widget, corner_radius=15, outline_color="#800000", outline_width=1):
...
def create_tab_background_with_text(text, button_width, button_height, selected=True, is_leftmost=False, is_rightmost=False):
    ...
    if text_width > button_width - 30:  # More generous padding allowance (increased from 20)
        words = text.split()
        if len(words) > 1:
            ...
            total_text_height = line_height * 2 + 5  # Increased gap between lines for better spacing
            ...
            draw.text((line2_x, start_y + line_height + 5), line2, font=font, fill=text_color)
        else:
            ...
            if text_width > button_width - 10:
                ...
                if smaller_width <= button_width - 10:
                    ...
                    draw.text((text_x, text_y), text, font=smaller_font, fill=text_color)
                else:
...
def create_tab_buttons(dashboard, parent=None):
    ...
    tabs_and_search_frame.pack(side=tk.TOP, fill=tk.X)  # Removed pady for seamless spacing
    
    # Create tab frame (left side)
    tab_frame = tk.Frame(tabs_and_search_frame, bg="#000000", height=button_height, width=total_button_width)
    ...
    def update_tab_styles(selected_button):
        ...
        for button, text, is_leftmost, is_rightmost in buttons_info:
            if button == selected_button:
                button_img = create_tab_background_with_text(text, button_width, button_height, selected=True, is_leftmost=is_leftmost, is_rightmost=is_rightmost)
                button.configure(bg="#000000", image=button_img, text="", activebackground="#000000")
                if button_img:
                    button.image = button_img
            else:
                button_img = create_tab_background_with_text(text, button_width, button_height, selected=False, is_leftmost=is_leftmost, is_rightmost=is_rightmost)
                button.configure(bg="#000000", image=button_img, text="", activebackground="#000000")
                if button_img:
                    button.image = button_img

    # Create initial button images
    materials_img = create_tab_background_with_text("Items List", button_width, button_height, selected=True, is_leftmost=True, is_rightmost=False)
    restock_img = create_tab_background_with_text("Restock List", button_width, button_height, selected=False, is_leftmost=False, is_rightmost=False)
    employee_logs_img = create_tab_background_with_text("Employee Logs", button_width, button_height, selected=False, is_leftmost=False, is_rightmost=False)
    admin_logs_img = create_tab_background_with_text("Admin Logs", button_width, button_height, selected=False, is_leftmost=False, is_rightmost=True)
    ...
    view_materials_list_button = tk.Button(
        ...
        command=lambda: [
            dashboard.view_items() if getattr(dashboard, 'current_view', None) != "ITEMS_LIST" else None,
            ...
            __import__('gui.functions.admdash_f.ML.search_bar', fromlist=['update_search_bar_visibility']).update_search_bar_visibility(dashboard),
        ]
    )
    ...
    restock_list_button = tk.Button(
        ...
        command=lambda: [
            dashboard.view_restock_list() if getattr(dashboard, 'current_view', None) != "Restock List" else None,
