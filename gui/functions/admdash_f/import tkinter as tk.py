...
class UserRolesManager:
    ...
    def _show_wizard_page(self, build_func, title="Wizard", **kwargs):
        ...
        self._wizard_prev_focus = self.settings_win.focus_get() if self.settings_win else None

        # Header bar
        header = tk.Frame(self._wizard_frame, bg="#181818")
        header.pack(fill='x')
        tk.Label(header, text=title, font=("Segoe UI", 14, "bold"), bg="#181818", fg="#ff8c00").pack(side='left', padx=12, pady=8)

        def close_wizard():
            self._hide_wizard()

        tk.Button(header, text="✕", command=close_wizard, bg="#3a3a3a", fg="#ffffff",
                  relief='flat', width=3).pack(side='right', padx=8, pady=6)
    ...
    def create_full_interface(self, parent):
        ...
        instructions_text = "Instructions: Right-click on any user to change their access level."
        tk.Label(instructions_frame, text=instructions_text, 
                font=("Segoe UI", 10), bg="#000000", fg="#999", wraplength=600).pack(anchor='w')
        
        # Search frame
        search_frame = tk.Frame(container, bg="#000000")
        search_frame.pack(fill='x', pady=(0, 10))
        
        search_label = tk.Label(search_frame, text="Search employees:", 
        ...
        search_entry = tk.Entry(search_frame, textvariable=self.search_var, 
                              font=("Segoe UI", 10), bg="#23272b", fg="#fffde7",
                              insertbackground="#fffde7", relief='flat', bd=1, width=30)
        search_entry.pack(side='left', padx=(10, 0), ipady=3)
        
        # Clear search button
        clear_btn = tk.Button(search_frame, text="Clear", font=("Segoe UI", 9),
        ...
        self.stats_label = tk.Label(stats_frame, text="", font=("Segoe UI", 9),
                                   bg="#000000", fg="#999")
        ...
        style.configure("Modified.Treeview",
                       background="#2d4a3e",  # Darker green background
                       foreground="#a8d8a8",  # Light green text
                       fieldbackground="#2d4a3e")
        
        # Map the modified style
        style.map("Modified.Treeview",
                 background=[('selected', '#4a6741')])  # Selected modified row color
        
        # Configure column headings with sorting
        self.user_tree.heading('Last Name', text='Last Name ↕', command=lambda: self.sort_treeview('Last Name'))
        self.user_tree.heading('First Name', text='First Name ↕', command=lambda: self.sort_treeview('First Name'))
        self.user_tree.heading('Access Level', text='Access Level ↕', command=lambda: self.sort_treeview('Access Level'))
        ...
        # Add Employee button
        self.add_emp_btn = tk.Button(emp_mgmt_frame,
                                   text="Add Employee",
                                   font=("Segoe UI", 10),
                                   bg="#28a745",
                                   fg="#fff",
        ...
        self.remove_emp_btn = tk.Button(emp_mgmt_frame,
        ...
        self.edit_emp_btn = tk.Button(emp_mgmt_frame,
                                    text="Edit Employee",
                                    font=("Segoe UI", 10),
                                    bg="#ffc107",
                                    fg="#000",
        ...
        self.bulk_ops_btn = tk.Button(emp_mgmt_frame,
        ...
        self.cancel_roles_btn = tk.Button(btn_frame, 
                                        text="Cancel", 
                                        font=("Segoe UI", 11), 
                                        bg="#666", 
                                        fg="#fff", 
        ...
        self.save_roles_btn = tk.Button(btn_frame,
                                      text="💾 Save Changes",
        ...
        self.user_tree.bind('<Button-3>', self.on_right_click)  # Right-click context menu
        ...
        self.user_tree.bind('<Control-p>', lambda e: self.show_pending_changes_summary())  # Ctrl+P to show pending changes
        
        # Double-click to edit
        self.user_tree.bind('<Double-1>', lambda e: self.edit_employee_dialog() if self.user_tree.selection() else None)
        ...
        shortcuts_label = tk.Label(shortcuts_frame, text=shortcuts_text, 
                                  font=("Segoe UI", 8), bg="#000000", fg="#666")
        ...
        changes_label = tk.Label(changes_info_frame, text=changes_text, 
                                font=("Segoe UI", 8), bg="#000000", fg="#888")
        changes_label.pack(anchor='w')

        # Schedule initial data load once UI is ready
        try:
            target = self.settings_win if hasattr(self, 'settings_win') and self.settings_win else container
    ...
    def load_user_data(self):
        ...
        try:
            ...
            db_path = os.path.join(base_dir, 'database', 'JJCIMS.accdb')
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT [Last Name], [First Name],  [Middle Name], [Username], [Access Level] FROM [Emp_List] ORDER BY [Last Name], [First Name]")
    ...
    def save_changes_test(self):
        ...
        try:
            ...
            for i, change in enumerate(changes):
                ...
                cursor.execute("UPDATE [Emp_list] SET [Access Level]=? WHERE [Username]=?", (change['new_level'], change['username']))
                conn.commit()
            cursor.close()
    ...
    def save_changes(self):
        ...
        for change in changes:
            print(f"[DEBUG] Change: {change['first_name']} {change['last_name']} ({change['old_level']} -> {change['new_level']})")
        ...
        try:
            ...
            for i, change in enumerate(changes):
                ...
                if change['old_level'] in ['Level 2', 'Level 3'] and change['new_level'] == 'Level 1':
                    ...
                    cursor.execute("UPDATE [Emp_list] SET [Access Level]=?, [Password]=?, [2FA Secret]=? WHERE [Username]=?",
                    ...
                    cursor.execute("UPDATE [Emp_list] SET [Access Level]=? WHERE [Username]=?",
...
• The user must be present to provide their credentials"""
    ...
    def verify_target_user_credentials_flexible(self, target_username, password, tfa_code, auth_method):
        ...
        try:
            ...
            conn = pyodbc.connect(conn_str)
            cursor = conn.cursor()
            cursor.execute("SELECT [Password], [2FA Secret] FROM [Emp_list] WHERE [Username]=?", (target_username,))
            row = cursor.fetchone()
            cursor.close()
    ...
    def check_username_exists(self, username):
        ...
        try:
            ...
            conn = pyodbc.connect(conn_str)
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM [Emp_list] WHERE [Username]=?", (username,))
