...
class IntegratedAdminSettings:
    ...
    def __init__(self, parent):
        self.parent = parent
        self.assets_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../assets/sett_assets'))
        self.secret_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../config/admin_otp_secret.txt'))
        
        # Bypass 2FA/secret check if username is 'bypass'
        if hasattr(parent, 'username') and getattr(parent, 'username', '').lower() == 'bypass':
    ...
    def show_settings(self):
        ...
        if self.settings_win and self.settings_win.winfo_exists():
            ...
            try:
                ...
                self.settings_win.after(200, lambda: self.settings_win.attributes('-topmost', False) if self.settings_win and self.settings_win.winfo_exists() else None)
            except Exception:
                pass
            return
        ...
        try:
            ...
            self.settings_win.after(100, lambda: self.settings_win.focus_force() if self.settings_win and self.settings_win.winfo_exists() else None)
            # Briefly set topmost to guarantee focus, then revert
            self.settings_win.attributes('-topmost', True)
            self.settings_win.after(250, lambda: self.settings_win.attributes('-topmost', False) if self.settings_win and self.settings_win.winfo_exists() else None)
        except Exception:
            pass
    ...
    def create_navigation_buttons(self):
        ...
        try:
            ...
            button_configs = [
                {"image": "button_1.png", "x": 1276.0, "y": 104.0, "width": 94.0, "height": 30.0, "command": self.show_about},             # About
                {"image": "button_2.png", "x": 1080.0, "y": 104.0, "width": 180.0, "height": 30.0, "command": self.show_backup_restore_section}, # Backup/Restore
                {"image": "button_3.png", "x": 902.0, "y": 104.0, "width": 162.0, "height": 30.0, "command": self.show_import_export},     # Import/Export
                {"image": "button_4.png", "x": 301.0, "y": 106.0, "width": 130.0, "height": 30.0, "command": self.show_user_roles},        # User Roles
                {"image": "button_5.png", "x": 159.0, "y": 106.0, "width": 126.0, "height": 30.0, "command": self.show_2fa_section},       # Setup 2FA
                {"image": "button_6.png", "x": 30.0, "y": 106.0, "width": 113.0, "height": 30.0, "command": self.show_account_settings}    # Account Settings
            ]
                ...
                try:
                    ...
                    if os.path.exists(image_path):
                        ...
                        button = tk.Button(
                            ...
                            command=lambda cmd=config["command"], btn=button_num: [cmd(), self.select_button(btn)],
                            relief="flat"
                        )
                    ...
                    button_names = ["About", "Backup/Restore", "Import/Export", "User Roles", "Setup 2FA", "Account Settings"]
    ...
    def enable_edit(self):
        ...
        for entry in [self.first_name_entry, self.last_name_entry, self.username_entry, self.password_entry]:
    ...
    def save_account_settings(self):
        ...
        try:
            ...
            key_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../config/fernet_key.py'))
            ...
            cursor.execute("""
                ...
                WHERE [Username]=?
            """, (first_name, last_name, username, encrypted_password, original_username))
    ...
    def create_import_export(self, parent):
        ...
        try:
            ...
            if module_path.exists():
                spec = importlib.util.spec_from_file_location('admin_import_export_section', module_path)
                mod = importlib.util.module_from_spec(spec)  # type: ignore
                try:
                    spec.loader.exec_module(mod)  # type: ignore
                    # Prefer factory if provided
                    if hasattr(mod, 'create_import_export_frame'):
                        self.import_export_view = mod.create_import_export_frame(container)
                    elif hasattr(mod, 'ImportExport'):
                        self.import_export_view = mod.ImportExport(container)
                    else:
                        raise AttributeError('No ImportExport class or factory found in i&e_section.py')
                    self.import_export_view.pack(fill='both', expand=True)
                except Exception as inner_e:
                    traceback.print_exc()
                    self._build_import_export_fallback(container, f"Failed loading enhanced Import/Export UI: {inner_e}")
            else:
                self._build_import_export_fallback(container, "Enhanced Import/Export file not found.")
        except Exception as e:
            self._build_import_export_fallback(container, f"Error initializing enhanced Import/Export UI: {e}")
        return container
    ...
    def _build_import_export_fallback(self, frame, error_msg=None):
        ...
        tk.Label(frame, text="Import / Export", font=("Segoe UI", 15, "bold"), bg="#000000", fg="#fffde7").pack(anchor='w', pady=(18, 8), padx=18)
        if error_msg:
            tk.Label(frame, text=error_msg, font=("Segoe UI", 10), bg="#000000", fg="#ff6f00", wraplength=800, justify='left').pack(anchor='w', padx=18, pady=(0,10))
        btn_frame = tk.Frame(frame, bg="#000000")
        btn_frame.pack(pady=18)
        tk.Button(btn_frame, text="Import Database", command=self.import_database, font=("Segoe UI", 12, "bold"), bg="#4CAF50", fg="white", relief='flat', width=16).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="Export Database", command=self.export_database, font=("Segoe UI", 12, "bold"), bg="#FF9800", fg="white", relief='flat', width=16).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="Import Logs", command=self.import_logs, font=("Segoe UI", 12, "bold"), bg="#9C27B0", fg="white", relief='flat', width=16).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_frame, text="Export Logs", command=self.export_logs, font=("Segoe UI", 12, "bold"), bg="#2196F3", fg="white", relief='flat', width=16).pack(side=tk.LEFT, padx=10)
        ...
        try:
            ...
            if source_file:
                if messagebox.askyesno("Confirm Import", 
                    ...
                    target_db = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../database/JJCIMS.accdb'))
    ...
    def export_database(self):
        ...
        try:
            ...
            if target_file:
                source_db = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../database/JJCIMS.accdb'))
                shutil.copy2(source_db, target_file)
                messagebox.showinfo("Success", f"Database exported successfully!\n{target_file}")
    ...
    def import_logs(self):
        ...
        try:
            ...
            if source_file:
                if messagebox.askyesno("Confirm Import", 
                                     "This will replace the current logs.\n"
                                     "Are you sure you want to continue?"):
                    
                    target_logs = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../database/logs.xlsx'))
                    backup_logs = target_logs + ".backup"
                    
                    # Backup current logs
                    if os.path.exists(target_logs):
                        shutil.copy2(target_logs, backup_logs)
                    
                    # Import new logs
                    shutil.copy2(source_file, target_logs)
    ...
    def export_logs(self):
        ...
        try:
            ...
            if target_file:
                source_logs = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../database/logs.xlsx'))
                if os.path.exists(source_logs):
                    shutil.copy2(source_logs, target_file)
                    messagebox.showinfo("Success", f"Logs exported successfully!\n{target_file}")
                else:
                    messagebox.showwarning("Warning", "No logs file found to export.")
    ...
    def create_2fa_section(self, parent):
        ...
        try:
            ...
            base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..'))
            db_path = os.path.join(base_dir, 'database', 'JJCIMS.accdb')
            fernet_key_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../config/fernet_key.py'))
            ...
            tk.Label(title_frame, text="Setup 2FA", font=("Segoe UI", 15, "bold"), bg="#000000", fg="#fffde7").pack()
            
            error_frame = tk.Frame(container, bg="#000000")
            error_frame.pack(fill='x', pady=20)
            tk.Label(error_frame, text=f"Error loading 2FA wizard: {str(e)}", font=("Segoe UI", 12), bg="#000000", fg="#ff6f00").pack()
    ...
    def create_backup_restore_section(self, parent):
        ...
        try:
            assets_backup = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../assets/bnr_assets/bu_assets'))
            assets_restore = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../assets/bnr_assets/r_assets'))
            
            # Create the BackupRestoreSection and pack it to fill the frame
            backup_restore_section = BackupRestoreSection(frame, assets_backup, assets_restore)
            ...
            title_label = tk.Label(container, text="Backup / Restore", 
                                 font=("Segoe UI", 15, "bold"), 
                                 bg="#000000", fg="#fffde7")
            title_label.pack(pady=(20, 10))
            
            error_label = tk.Label(container, text=f"Error loading backup/restore interface:\n{str(e)}", 
                                 font=("Segoe UI", 11), 
                                 bg="#000000", fg="#ff6f00")
            ...
            backup_btn = tk.Button(btn_frame, text="Create Backup", 
                                 font=("Segoe UI", 12, "bold"), 
                                 bg="#4CAF50", fg="white", relief='flat', 
                                 width=16, command=self.create_backup_fallback)
            backup_btn.pack(side=tk.LEFT, padx=10)
            
            restore_btn = tk.Button(btn_frame, text="Restore Backup", 
                                  font=("Segoe UI", 12, "bold"), 
                                  bg="#FF9800", fg="white", relief='flat', 
                                  width=16, command=self.restore_backup_fallback)
