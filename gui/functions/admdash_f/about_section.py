...
from utils.notification_manager import init_notifications, show_notification
...
def create_text_with_glow(text, font_size, is_bold=False):
    ...
    # Main text
    draw.text((padding, padding), text, font=font, fill=(255, 111, 0, 255))  # Solid orange
...
No scientific data gained. No protocol breached. No reward issued."""
    ...
    prev_btn = tk.Button(nav_frame, text="<", command=prev_page, 
                        bg="#000000", fg="#fffde7", font=("Segoe UI", 12),
                        width=3, relief="flat", borderwidth=0)
    prev_btn.pack(side='left', padx=5)

    page_label = tk.Label(nav_frame, textvariable=current_page, 
                         bg="#000000", fg="#fffde7", font=("Segoe UI", 12))
    page_label.pack(side='left', padx=10)

    next_btn = tk.Button(nav_frame, text=">", command=next_page,
                        bg="#000000", fg="#fffde7", font=("Segoe UI", 12),
                        width=3, relief="flat", borderwidth=0)
    ...
    try:
        logo_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../assets/JJCIMS.png'))
    ...
    version = tk.Label(content_frame,
    ...
    company = tk.Label(content_frame,
    ...
    tk.Label(dev_frame,
            ...
            font=("Segoe UI", 16),  # Increased from 12 to 16
            bg="#000000",
            fg="#fffde7").pack(side=tk.LEFT)

    # Clickable developer name with link
    dev_link = tk.Label(dev_frame,
    ...
    dev_link.bind("<Button-1>", lambda e: webbrowser.open("https://www.instagram.com/just.kwf/"))
    ...
    # Aliases with email
    aliases = tk.Label(contact_frame,
    ...
    email_link = tk.Label(contact_frame,
    ...
    email_link.bind("<Button-1>", lambda e: webbrowser.open("https://mail.google.com/mail/u/0/?fs=1&to=Keidesu02@gmail.com&su=Reaching%20Out%20from%20JJC%20FPIS&body=Hello%20there!%0A%0AI%20am%20reaching%20out%20regarding%20our%20JJC%20FPIS%20project.%20Let%20me%20know%20when%20you're%20available%20to%20discuss.%0A%0AThank%20you!&tf=cm"))
    ...
    try:
        ...
        lambda_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../assets/Lambda_logo.png'))
        ...
        lambda_img = lambda_img.resize((lambda_size, lambda_size), Image.Resampling.LANCZOS)
        
        # Use logo without scanline effect
        lambda_photo = ImageTk.PhotoImage(lambda_img)
        
        # Lambda logo replacing the © symbol (clickable easter egg)
        lambda_copyright = tk.Label(copyright_frame, image=lambda_photo, bg="#000000", cursor="hand2")
    ...
    desc_label = tk.Label(content_frame2, text=description, font=("Segoe UI", 16),  # Increased from 12 to 16
                         bg="#000000", fg="#fffde7", wraplength=600, justify="center")  # Increased wraplength
    ...
    try:
        ...
        lambda_bullet_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../assets/Lambda_logo.png'))
        ...
        lambda_bullet_img = lambda_bullet_img.resize((lambda_bullet_size, lambda_bullet_size), Image.Resampling.LANCZOS)
        ...
        for i, feature in enumerate(features):
            if i == 5:  # Replace bullet for "Audit logging" with lambda
                ...
                lambda_bullet = tk.Label(feature_frame, image=lambda_bullet_photo, bg="#000000", cursor="hand2")
                ...
                feature_text = tk.Label(feature_frame, text=feature[2:], font=("Segoe UI", 12),  # Remove "• " from start
                                       bg="#000000", fg="#fffde7")
                feature_text.pack(side='left')
            else:
                # Normal feature display
                feature_label = tk.Label(content_frame2, text=feature, font=("Segoe UI", 12),
                                       bg="#000000", fg="#fffde7")
