...
from database import get_connector, get_db_path  # centralized DB access
...
def add_item(root, db, load_data_callback, current_view=None, username="Admin"):
    ...
    def add_material_to_jjcims(root, db, load_data_callback):
        ...
        add_window.after(10, add_window.deiconify)  # Show window again to register with system
        ...
        if db is None or not hasattr(db, 'connect'):
            try:
                ...
                try:
                    ...
                    messagebox.showerror("Database Error", "Unable to initialize database connector.")
                    add_window.destroy()
                    return
                ...
                try:
                    ...
                    if "adm_logs" in str(insert_err).lower():
                        try:
                            ...
                            print(f"[LOGGING] Failed creating adm_logs table: {create_err}")
                    else:
        ...
        try:
            header_image = ImageTk.PhotoImage(Image.open(relative_to_assets("image_1.png")))
        ...
        for i in range(1, 13):
            try:
                img = ImageTk.PhotoImage(Image.open(relative_to_assets(f"button_{i}.png")))
    ...
    # handle_button_click defined later with additional actions (close/minimize/fullscreen)
        ...
        for i in range(1, 10):
            try:
                entry_img = ImageTk.PhotoImage(Image.open(relative_to_assets(f"entry_{i}.png")))
            ...
            canvas.create_image(
                x + w/2,
                y + h/2,
                image=add_window.image_references[f'entry_{i}']
            )
            ...
            if i in [2, 3, 4, 7, 9]:  # Brand, Type, Location, Unit of Measure, Supplier
                ...
                entry.bind('<<ComboboxSelected>>', 
                    lambda e, combo=entry, field=field_map[i]: 
                        handle_combo_selection(e, combo, field, db, add_window))
        ...
        button_7_image = ImageTk.PhotoImage(Image.open(relative_to_assets("button_7.png")))
        button_7 = tk.Button(
            add_window,
        ...
        def handle_button_click(button_num):
            if button_num == 1:  # Red - Close
                ...
                def color_wave():
                    colors = ["#800000", "#A00000", "#C00000", "#E00000", "#FF0000", "#E00000", "#C00000", "#A00000"]
                    def wave_step(color_index=0):
                        if header_frame.winfo_exists():
                            ...
                            if color_index < len(colors) - 1:
                                ...
                                add_window.after(400, lambda: header_frame.configure(bg="#800000"))
                    wave_step()
                ...
                if not getattr(add_window, 'fullscreen', False):
                    ...
                    if hasattr(add_window, '_normal_geometry') and add_window._normal_geometry:
                        add_window.geometry(add_window._normal_geometry)
                    add_window.fullscreen = False
        ...
        def save_item():
            ...
            if not all(values.values()):
                ...
                title_label = tk.Label(title_bar, text="Error", bg="#800000", fg="white", font=("Inter", 10))
                ...
                # Message
                msg_label = tk.Label(error_dialog, text="All fields are required", bg="#000000", fg="white", 
                ...
                # OK button
                ok_btn = tk.Button(button_frame, text="OK", command=error_dialog.destroy,
                ...
                x = add_window.winfo_x() + (add_window.winfo_width() - error_dialog.winfo_width()) // 2
                y = add_window.winfo_y() + (add_window.winfo_height() - error_dialog.winfo_height()) // 2
                error_dialog.geometry(f"+{x}+{y}")
            ...
            try:
                ...
                cursor.execute("SELECT NAME FROM ITEMSDB WHERE NAME = ?", (values["NAME"],))
                if cursor.fetchone():
                    ...
                    # Message
                    msg_label = tk.Label(error_dialog, text=f"Item '{values['NAME']}' already exists!", 
                                       bg="#000000", fg="white", font=("Inter", 10), wraplength=300)
                    msg_label.pack(padx=20, pady=20)
                ...
                """, (
                    values["NAME"], values["BRAND"], values["TYPE"], values["LOCATION"],
                    values["UNIT OF MEASURE"], int(values["IN"]), int(values["MIN STOCK"]),
                    float(values["PRICE PER UNIT"]), values["SUPPLIER"]
                ))
                connection.commit()
                ...
                try:
                    ...
                    print(f"[WARNING] Access queries failed, using fallback: {query_error}")
                    ...
                    try:
                        ...
                        print(f"[ERROR] Both Access queries and fallback SQL failed: {fallback_error}")
                ...
                title_label = tk.Label(title_bar, text="Success", bg="#800000", fg="white", font=("Inter", 10))
                title_label.pack(side=tk.LEFT, padx=10, pady=5)
                ...
                # Message
                msg_label = tk.Label(success_dialog, text="Item added successfully!", 
                                   bg="#000000", fg="white", font=("Inter", 10), wraplength=300)
                msg_label.pack(padx=20, pady=20)
                ...
                # OK button
                ok_btn = tk.Button(button_frame, text="OK", command=success_dialog.destroy,
                                 bg="#800000", fg="white", font=("Inter", 10), width=8)
                ok_btn.pack(padx=5)
                ...
                x = add_window.winfo_x() + (add_window.winfo_width() - success_dialog.winfo_width()) // 2
                y = add_window.winfo_y() + (add_window.winfo_height() - success_dialog.winfo_height()) // 2
                success_dialog.geometry(f"+{x}+{y}")
                ...
                try:
                    cursor = connection.cursor()
                    cursor.execute("DELETE FROM ANI_DRAFTS WHERE [Item Name] = ?", (values["NAME"],))
                    connection.commit()
                except Exception as e:
                    # Only log serious errors, ignore table not found
                    if not str(e).startswith("('42S02'"):  # Not a "table not found" error
                ...
                title_label = tk.Label(title_bar, text="Add Another Item", bg="#800000", fg="white", font=("Inter", 10))
                title_label.pack(side=tk.LEFT, padx=10, pady=5)
                ...
                # Message
                msg_label = tk.Label(dialog, text="Would you like to add another item?", 
                                   bg="#000000", fg="white", font=("Inter", 11), wraplength=250)
                msg_label.pack(padx=20, pady=20)
                ...
                yes_btn = tk.Button(btn_frame, text="Yes", command=on_yes, 
                                  bg="#800000", fg="white", font=("Inter", 10), width=10)
                ...
                dialog_width = 300
                dialog_height = 150
                x = add_window.winfo_x() + (add_window.winfo_width() - dialog_width) // 2
                y = add_window.winfo_y() + (add_window.winfo_height() - dialog_height) // 2
                ...
                # Message
                msg_label = tk.Label(error_dialog, text="Please enter valid numbers for quantity and price fields", 
                                   bg="#000000", fg="white", font=("Inter", 10), wraplength=300)
                msg_label.pack(padx=20, pady=20)
                ...
                # Message
                msg_label = tk.Label(error_dialog, text=f"An error occurred: {str(e)}", 
                                   bg="#000000", fg="white", font=("Inter", 10), wraplength=300)
                msg_label.pack(padx=20, pady=20)
...
def load_combo_data(db, field):
    ...
    try:
        connection = db.connect()
        cursor = connection.cursor()
        cursor.execute(f"SELECT DISTINCT [{field}] FROM ITEMSDB WHERE [{field}] IS NOT NULL ORDER BY [{field}]")
    ...
    def handle_input(event=None):
        ...
        if new_value and new_value != "Add +":
            ...
            try:
                connection = db.connect()
                cursor = connection.cursor()
                cursor.execute(f"SELECT COUNT(*) FROM ITEMSDB WHERE [{field}] = ?", (new_value,))
...
def handle_combo_selection(event, combo, field, db, add_window):
    ...
    if combo.get() == "Add +":
        ...
        try:
            ...
            cursor.execute(f"SELECT COUNT(*) FROM ITEMSDB WHERE [{field}] = ?", (combo.get(),))
            count = cursor.fetchone()[0]
...
def handle_helper_button(button_num, entries, db, add_window):
    ...
    if button_num in button_map:
        ...
        for i in [1,5,6,8]:  # Only bind to non-combo entries
            ...
            entry.bind('<KeyRelease>', lambda e, num=i: validate_entry(num, e.widget.get()))
            entry.bind('<FocusOut>', lambda e, num=i: validate_entry(num, e.widget.get()))
