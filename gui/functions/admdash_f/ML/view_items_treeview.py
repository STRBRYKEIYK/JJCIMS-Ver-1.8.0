...
DEFAULT_COLUMN_WIDTHS = {
    "NAME": 150, "BRAND": 100, "TYPE": 100, "LOCATION": 100, "UNIT OF MEASURE": 100, "STATUS": 80,
    "IN": 60, "OUT": 60, "BALANCE": 70, "MIN STOCK": 80, "DEFICIT": 70, "PRICE PER UNIT": 100, "COST": 100, "LAST PO": 100, "SUPPLIER": 120
}
NUMERIC_COLUMNS = {"IN", "OUT", "BALANCE", "MIN STOCK", "DEFICIT", "PRICE PER UNIT", "COST"}
CENTERED_COLUMNS = {"IN", "OUT", "BALANCE", "MIN STOCK", "DEFICIT", "STATUS", "PRICE PER UNIT", "COST", "LAST PO"}
...
def view_items_treeview(dashboard):
    ...
    dashboard.scroll_x = ttk.Scrollbar(dashboard.table_frame, orient=tk.HORIZONTAL, style="Custom.Horizontal.TScrollbar")
    
    # Store sort state
    dashboard.sort_states = {col: False for col in columns}  # False = ascending, True = descending
    ...
    def sort_column(col):
        ...
        if hasattr(dashboard, 'current_view') and dashboard.current_view == "Restock List":
        ...
        # Get all items
        rows_list = [(dashboard.table.set(k, col), k) for k in dashboard.table.get_children('')]
        
        # Convert numbers to float for proper numeric sorting
        if col in NUMERIC_COLUMNS:
            rows_list = [(float(''.join(filter(str.isdigit, i[0]))) if i[0].strip() else 0, i[1]) for i in rows_list]
        # Special handling for LAST PO column
            ...
            rows_list = [(parse_date(i[0]) if i[0].strip() else datetime.min, i[1]) for i in rows_list]
    ...
    for col, width, anchor in col_settings:
        ...
        dashboard.table.heading(col, text=col, anchor=anchor, command=make_sort_command())
        dashboard.table.column(col, width=width, anchor=anchor, minwidth=60, stretch=False)
    ...
    items = [(dashboard.table.set(row_id, "NAME"), row_id) for row_id in dashboard.table.get_children("")]
