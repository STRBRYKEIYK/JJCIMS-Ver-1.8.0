...
def get_unit_of_measure(connector, name):
    ...
    row = connector.fetchone(
        "SELECT [UNIT OF MEASURE] FROM ITEMSDB WHERE [NAME] = ?", (name,)
    )
    if row and row[0]:
        return row[0]
    return None
...
def get_emp_2fa_and_access(connector, username_lower):
    ...
    return connector.fetchone(
        "SELECT [2FA Secret], [Access Level] FROM [emp_list] WHERE LCase([Username])=?",
        (username_lower,),
    )
...
def fetch_items_by_type(connector, category):
    ...
    return connector.fetchall(
        "SELECT ID, NAME, BRAND, TYPE, LOCATION, UNIT_OF_MEASURE, STATUS, BALANCE FROM ITEMSDB WHERE TYPE = ?",
        (category,),
    )
...
def get_user_by_username(connector, username):
    ...
    return connector.fetchone(
        "SELECT * FROM [emp_list] WHERE [Username]=?", (username,)
    )
...
def get_user_by_username_lower(connector, username_lower):
    ...
    return connector.fetchone(
        "SELECT * FROM [emp_list] WHERE LCase([Username])=?", (username_lower,)
    )
...
def update_user_password(connector, username_lower, encrypted_password):
    ...
    connector.execute_query(
        ...
        (encrypted_password, username_lower),
    )
